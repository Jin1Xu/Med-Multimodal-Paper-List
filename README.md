# Med-Multimodal-Paper-List
This repo provides a curated collection of **Medical-focused MultiModal and Cross-Modal** research papers accepted at leading ML and CV conferences between **2024 and 2025**. Contributions, corrections, and additional paper entries are welcome.

including: **AAAI, NeurIPS, ICML, ICLR, IJCAI, ECCV, ICCV, CVPR, ISBI, MICCAI**

Deadline Info for Conferences: [https://ccfddl.com/](https://ccfddl.com/)   and Ours: [https://jiandanjinxin.github.io/](https://jiandanjinxin.github.io/)

[**Paper Copilot**](https://papercopilot.com/paper-list/) & [Github](https://github.com/papercopilot/paperlists/tree/main) 提供了绝大部分完整的顶会文献列表🌹  

## 📅 Conference-List
| Conference                                        | Type | Submission    | Conference Dates | Papers           |
|:-------------------------------------------------:|:---:|:---------------|:-----------------|:-----------------|
| [AAAI](https://aaai.org/conference/aaai/aaai-26/) | ML  | Aug 2 2025     |    -             | -                |
| [NeurIPS](https://neurips.cc/Conferences/2025)    | ML  | May 11 2025    | Dec 2025         | -                |
| [ICML](https://icml.cc/)                          | ML  | Jan 23 2026    | Jul 2026         | -                |
| [ICLR](https://openreview.net/group?id=ICLR.cc/2025/Conference#tab-accept-oral)  | ML | Sep 19 2025 | Apr 2026 | - |
| [IJCAI](https://2025.ijcai.org/)                  | ML  | Jan 19 2026    | Aug 2026         | -                |
| [**ECCV**](https://eccv.ecva.net/Conferences/2026)| CV  | **Mar 6 2026** | Sep 2026         | -                |
| [ICCV](https://iccv.thecvf.com/Conferences/2025)  | CV  | Mar 7 2025     | Oct 2025         | -                |
| [CVPR](https://cvpr.thecvf.com/Conferences/2025)  | CV  | Nov 7 2025     | Jun 2026         | -                |
| [ISBI](https://biomedicalimaging.org/2025/)       | Med | Nov 14 2025    | Apr 2026         | https://dblp.org/db/conf/isbi/isbi2025.html |
| [**MICCAI**](https://conferences.miccai.org/2026/en/default.asp) | Med | **Feb 12 2026** | Oct 2026 | https://papers.miccai.org/miccai-2025/ |


## ✅ MM-Classification-Paper-List
聚焦于**多模态分类**任务的论文
<!-- MM_CLASS_PAPERS_START -->

<details>
  <summary><strong>NIPS 2025</strong>（共筛选出 42 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 22O1ejTxj3 | [Modality-Aware SAM: Sharpness-Aware-Minimization Driven Gradient Modulation for Harmonized Multimodal Learning](https://openreview.net/forum?id=22O1ejTxj3) | Hossein R. Nowdeh | Poster |  | 😾 | 
| 38n8pFvldK | [CroPe: Cross-Modal Semantic Compensation Adaptation for All Adverse Scene Understanding](https://openreview.net/forum?id=38n8pFvldK) | Qin Xu | Poster |  | 😾 | 
| 3vLp3J7540 | [A Multimodal BiMamba Network with Test-Time Adaptation for Emotion Recognition Based on Physiological Signals](https://openreview.net/forum?id=3vLp3J7540) | Ziyu Jia | Poster |  | 😾 | 
| 6vI3OOYddm | [MolVision: Molecular Property Prediction with Vision Language Models](https://openreview.net/forum?id=6vI3OOYddm) | Deepan Adak | Poster |  | 😾 | 
| Bsska2ayiy | [MLEP: Multi-granularity Local Entropy Patterns for Generalized AI-generated Image Detection](https://openreview.net/forum?id=Bsska2ayiy) | Lin Yuan | Poster |  | 😾 | 
| CBsANtjBV4 | [Mitigating Intra- and Inter-modal Forgetting in Continual Learning of Unified Multimodal Models](https://openreview.net/forum?id=CBsANtjBV4) | Xiwen Wei | Poster |  | 😾 | 
| E6SFbnPiVP | [FedMGP: Personalized Federated Learning with Multi-Group Text-Visual Prompts](https://openreview.net/forum?id=E6SFbnPiVP) | Weihao Bo | Poster |  | 😾 | 
| EYC29OdxgA | [Structure-Aware Fusion with Progressive Injection for Multimodal Molecular Representation Learning](https://openreview.net/forum?id=EYC29OdxgA) | Zihao Jing | Poster |  | 😾 | 
| Fj2kyLs6G5 | [NeuroH-TGL: Neuro-Heterogeneity Guided Temporal Graph Learning Strategy for Brain Disease Diagnosis](https://openreview.net/forum?id=Fj2kyLs6G5) | Shengrong Li | Poster |  | 😾 | 
| GiUI70epUN | [$\texttt{AVROBUSTBENCH}$: Benchmarking the Robustness of Audio-Visual Recognition Models at Test-Time](https://openreview.net/forum?id=GiUI70epUN) | Sarthak Kumar Maharana | Poster |  | 😾 | 
| KkChcYqvdg | [egoEMOTION: Egocentric Vision and Physiological Signals for Emotion and Personality Recognition in Real-world Tasks](https://openreview.net/forum?id=KkChcYqvdg) | Matthias Jammot | Poster |  | 😾 | 
| KkONOZwvSU | [Defending Multimodal Backdoored Models by Repulsive Visual Prompt Tuning](https://openreview.net/forum?id=KkONOZwvSU) | Zhifang Zhang | Poster |  | 😾 | 
| MY4KpJt3LM | [LVLM-Driven Attribute-Aware Modeling for Visible-Infrared Person Re-Identification](https://openreview.net/forum?id=MY4KpJt3LM) | Zhiqi Pang | Poster |  | 😾 | 
| P4KjfMrVNU | [Hierarchical Information Aggregation for Incomplete Multimodal Alzheimer's Disease Diagnosis](https://openreview.net/forum?id=P4KjfMrVNU) | Chengliang Liu | Poster |  | 😾 | 
| Q6IyUpBmrG | [Rethinking Multimodal Learning from the Perspective of Mitigating Classification Ability Disproportion](https://openreview.net/forum?id=Q6IyUpBmrG) | Qing-Yuan Jiang | Oral |  | 😾 | 
| QKOdOlfzSe | [NAUTILUS: A Large Multimodal Model for Underwater Scene Understanding](https://openreview.net/forum?id=QKOdOlfzSe) | Wei Xu | Poster |  | 😾 | 
| RDdfQc5Ts1 | [AOR: Anatomical Ontology-Guided Reasoning for Medical Large Multimodal Model in Chest X-Ray Interpretation](https://openreview.net/forum?id=RDdfQc5Ts1) | Qingqiu Li | Poster |  | 😾 | 
| SeC5Zb8Orf | [Knowledge Graph Enhanced Generative Multi-modal Models for Class-Incremental Learning](https://openreview.net/forum?id=SeC5Zb8Orf) | Xusheng Cao | Poster |  | 😾 | 
| TjWdyVWBAG | [ExGra-Med: Extended Context Graph Alignment for Medical Vision-Language Models](https://openreview.net/forum?id=TjWdyVWBAG) | Duy Minh Ho Nguyen | Poster |  | 😾 | 
| V8sDJ1oMEy | [VT-FSL: Bridging Vision and Text with LLMs for Few-Shot Learning](https://openreview.net/forum?id=V8sDJ1oMEy) | Wenhao Li | Poster |  | 😾 | 
| VNTj7PGlrz | [Unlabeled Data Improves Fine-Grained Image Zero-shot Classification with Multimodal LLMs](https://openreview.net/forum?id=VNTj7PGlrz) | Yunqi Hong | Poster |  | 😾 | 
| VkSd42HWil | [Dual-Res Tandem Mamba-3D: Bilateral Breast Lesion Detection and Classification on Non-contrast Chest CT](https://openreview.net/forum?id=VkSd42HWil) | Jiaheng Zhou | Poster |  | 😾 | 
| WQq5JPGQ0C | [RadZero: Similarity-Based Cross-Attention for Explainable Vision-Language Alignment in Chest X-ray with Zero-Shot Multi-Task Capability](https://openreview.net/forum?id=WQq5JPGQ0C) | Jonggwon Park | Poster |  | 😾 | 
| YAycrn1Idm | [MIDAS: Misalignment-based Data Augmentation Strategy for Imbalanced Multimodal Learning](https://openreview.net/forum?id=YAycrn1Idm) | Seong-Hyeon Hwang | Poster |  | 😾 | 
| bxELEjg3VE | [GSAlign: Geometric and Semantic Alignment Network for Aerial-Ground Person Re-Identification](https://openreview.net/forum?id=bxELEjg3VE) | Qiao Li | Poster |  | 😾 | 
| dw9H08UxJb | [Omni-Mol: Multitask Molecular Model for Any-to-any Modalities](https://openreview.net/forum?id=dw9H08UxJb) | Chengxin Hu | Poster |  | 😾 | 
| idtZwmjakN | [GEM: Empowering MLLM for Grounded ECG Understanding with Time Series and Images](https://openreview.net/forum?id=idtZwmjakN) | Xiang Lan | Poster |  | 😾 | 
| k71nsscO9b | [Adaptive Re-calibration Learning for Balanced Multimodal Intention Recognition](https://openreview.net/forum?id=k71nsscO9b) | Qu Yang | Poster |  | 😾 | 
| kkhRTTmXFV | [Local-Global Coupling Spiking Graph Transformer for Brain Disorders Diagnosis from Two Perspectives](https://openreview.net/forum?id=kkhRTTmXFV) | Geng Zhang | Poster |  | 😾 | 
| lTAtQW1Ufi | [From Human Attention to Diagnosis: Semantic Patch-Level Integration of Vision-Language Models in Medical Imaging](https://openreview.net/forum?id=lTAtQW1Ufi) | Dmitry Lvov | Poster |  | 😾 | 
| mI9HqgVuTS | [Federated Dialogue-Semantic Diffusion for Emotion Recognition under Incomplete Modalities](https://openreview.net/forum?id=mI9HqgVuTS) | Xihang Qiu | Poster |  | 😾 | 
| n6SrVj7I0g | [MoME: Mixture of Matryoshka Experts for Audio-Visual Speech Recognition](https://openreview.net/forum?id=n6SrVj7I0g) | Umberto Cappellazzo | Poster |  | 😾 | 
| oHbVboLXz6 | [Uni-MuMER: Unified Multi-Task Fine-Tuning of Vision-Language Model for Handwritten Mathematical Expression Recognition](https://openreview.net/forum?id=oHbVboLXz6) | Yu Li | Spotlight |  | 😾 | 
| rGWjTlK6Ev | [STARC-9: A Large-scale Dataset for Multi-Class Tissue Classification for CRC Histopathology](https://openreview.net/forum?id=rGWjTlK6Ev) | Barathi Subramanian | Poster |  | 😾 | 
| rOR5IZcwJx | [Robust SuperAlignment: Weak-to-Strong Robustness Generalization for Vision-Language Models](https://openreview.net/forum?id=rOR5IZcwJx) | Junhao Dong | Spotlight |  | 😾 | 
| sQO1ZEQGqX | [DisasterM3: A Remote Sensing Vision-Language Dataset for Disaster Damage Assessment and Response](https://openreview.net/forum?id=sQO1ZEQGqX) | Junjue Wang | Poster |  | 😾 | 
| t04casiy7F | [Structured Spectral Reasoning for Frequency-Adaptive Multimodal Recommendation](https://openreview.net/forum?id=t04casiy7F) | Wei Yang | Poster |  | 😾 | 
| vAxGuGmshO | [AlignVLM: Bridging Vision and Language Latent Spaces for Multimodal Document Understanding](https://openreview.net/forum?id=vAxGuGmshO) | Ahmed Masry | Poster |  | 😾 | 
| w1FUXt3ujK | [Prot2Text-V2: Protein Function Prediction with Multimodal Contrastive Alignment](https://openreview.net/forum?id=w1FUXt3ujK) | Xiao Fei | Poster |  | 😾 | 
| xLFYd1owiP | [Spot the Fake: Large Multimodal Model-Based Synthetic Image Detection with Artifact Explanation](https://openreview.net/forum?id=xLFYd1owiP) | Siwei Wen | Poster |  | 😾 | 
| yHi8Ao6GAe | [MAPLE: Multi-scale Attribute-enhanced Prompt Learning for Few-shot Whole Slide Image Classification](https://openreview.net/forum?id=yHi8Ao6GAe) | Junjie Zhou | Poster |  | 😾 | 
| z2SGaPIhLT | [SGCD: Stain-Guided CycleDiffusion for Unsupervised Domain Adaptation of Histopathology Image Classification](https://openreview.net/forum?id=z2SGaPIhLT) | Hsi-Ling Chen | Spotlight |  | 😾 | 
</details>

<details>
  <summary><strong>MICCAI 2025</strong>（共筛选出 184 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
|  | A Causal-holistic Adaptive Intervention Network for Tailoring Automated Coronary Artery Disease Diagnosis to Individual Patients |  |  |  | 😾 | 
|  | A Holistic Time-Aware Classification Model for Multimodal Longitudinal Patient Data |  |  |  | 😾 | 
|  | A Novel ED Triage Framework Using Conditional Imputation, Multi-Scale Semantic Learning, and Cross-Modal Fusion |  |  |  | 😾 | 
|  | AEM: Attention Entropy Maximization for Multiple Instance Learning based Whole Slide Image Classification |  |  |  | 😾 | 
|  | Ada-FCN: Adaptive Frequency-Coupled Network for fMRI-Based Brain Disorder Classification |  |  |  | 😾 | 
|  | Adapting Foundation Model for Dental Caries Detection with Dual-View Co-Training |  |  |  | 😾 | 
|  | Adaptive Graph Learning with Multi-Graph Convolutions for Brain Disorder Classification |  |  |  | 😾 | 
|  | Alzheimer’s Disease Recognition Based on Adaptive Graph Normalization Flow for Incomplete Multimodal Data Fusion |  |  |  | 😾 | 
|  | An Anatomical Significance-Aware Architecture for Explainable Myocardial Infarction Prediction via Multi-Task Learning |  |  |  | 😾 | 
|  | Analysis of Image-and-Text Uncertainty Propagation in Multimodal Large Language Models with Cardiac MR-Based Applications |  |  |  | 😾 | 
|  | Anatomical Graph-based Multilevel Distillation for Robust Alzheimer’s Disease Diagnosis with Missing Modalities |  |  |  | 😾 | 
|  | Anatomy-Guided Multimodal Graph Networks for Alzheimer’s Disease: Integrative Analysis of Cross-Modal Brain Connectivity Signatures |  |  |  | 😾 | 
|  | Asynchronous Multi-Modal Learning for Dynamic Risk Monitoring of Acute Respiratory Distress Syndrome in Intensive Care Units |  |  |  | 😾 | 
|  | Attention-Based Multimodal Deep Learning Model for Post-Stroke Motor Impairment Prediction |  |  |  | 😾 | 
|  | Automated Characterization of Myocardial Scar Topological Patterns for Ventricular Tachycardia Screening |  |  |  | 😾 | 
|  | Automated Detection of BK Virus in H&E Whole-Slide Images Using Weakly-Supervised Deep Learning and Interpretable Morphological Biomarkers |  |  |  | 😾 | 
|  | Background-Invariant Independence-Guided Multi-head Attention Network for Skin Lesion Classification |  |  |  | 😾 | 
|  | Brain-Heart-Gut Guided Multi-Constraint Knowledge Distillation for Early Alzheimer’s Disease Diagnosis |  |  |  | 😾 | 
|  | BrainAlign: EEG-Vision Alignment via Frequency-Aware Temporal Encoder and Differentiable Cluster Assigner |  |  |  | 😾 | 
|  | BrainPrompt: Multi-Level Brain Prompt Enhancement for Neurological Condition Identification |  |  |  | 😾 | 
|  | Bridging Radiological Images and Factors with Vision-Language Model for Accurate Diagnosis of Proliferative Hepatocellular Carcinoma |  |  |  | 😾 | 
|  | CARE-VL: A Domain-Specialized Vision-Language Model for Early ASD Screening |  |  |  | 😾 | 
|  | CBrain: Cross-Modal Learning for Brain Vigilance Detection in Resting-State fMRI |  |  |  | 😾 | 
|  | CLIMD: A Curriculum Learning Framework for Imbalanced Multimodal Diagnosis |  |  |  | 😾 | 
|  | CLIP-DSA: Textual Knowledge-Guided Cerebrovascular Diseases Recognition in Multi-View Digital Subtraction Angiography |  |  |  | 😾 | 
|  | CXR-CML: Improved zero-shot classification of long-tailed multi-label diseases in Chest X-Rays |  |  |  | 😾 | 
|  | CardiacCLIP: Video-based CLIP Adaptation for LVEF Prediction in a Few-shot Manner |  |  |  | 😾 | 
|  | Cerebrovascular Diseases Screening from Color Fundus Photography via Cross-View Fusion and Graph-Based Discrimination |  |  |  | 😾 | 
|  | Class-Conditioned Image Synthesis with Diffusion for Imbalanced Diabetic Retinopathy Grading |  |  |  | 😾 | 
|  | ClinGRAD: Clinically-Guided Genomics and Radiomics Interpretable GNN for Dementia Diagnosis |  |  |  | 😾 | 
|  | Clinical Data-Driven Retrieval-Augmented Model for Lung Nodule Malignancy Prediction |  |  |  | 😾 | 
|  | Clinical Prior Guided Cross-Modal Hierarchical Fusion for Histological Subtyping of Lung Cancer in CT Scans |  |  |  | 😾 | 
|  | CoCa-CXR: Contrastive Captioners Learn Strong Temporal Structures for Chest X-Ray Vision-Language Understanding |  |  |  | 😾 | 
|  | CoPA: Hierarchical Concept Prompting and Aggregating Network for Explainable Diagnosis |  |  |  | 😾 | 
|  | Collateral Circulation guided Multi-modality Fusion Network for Postoperative Infarct Prediction |  |  |  | 😾 | 
|  | Contrastive Masked Video Modeling for Coronary Angiography Diagnosis |  |  |  | 😾 | 
|  | Core-Periphery Principle Guided State Space Model for Functional Connectome Classification |  |  |  | 😾 | 
|  | Cross-Modal Brain Graph Transformer via Function-Structure Connectivity Network for Brain Disease Diagnosis |  |  |  | 😾 | 
|  | Cross-Modal CXR-CTPA Knowledge Distillation using latent diffusion priors towards CXR Pulmonary Embolism Diagnosis |  |  |  | 😾 | 
|  | Cross-Modal Contrastive Learning for Emotion Recognition: Aligning ECG with EEG-Derived Features |  |  |  | 😾 | 
|  | Deep Knowledge-Infused Transformer for NSCLC Lymph Node Station Metastasis Prediction: Development of an AI-Powered Intraoperative Decision System |  |  |  | 😾 | 
|  | Disentangled and Interpretable Multimodal Attention Fusion for Cancer Survival Prediction |  |  |  | 😾 | 
|  | Domain Generalization for Mammogram Classification by Suppressing Domain-Specific Features |  |  |  | 😾 | 
|  | Dual Correlation-aware Mamba for Microvascular Obstruction Identification in Non-contrast Cine Cardiac Magnetic Resonance |  |  |  | 😾 | 
|  | Dual Selective Gleason Pattern-Aware Multiple Instance Learning for Grade Group Prediction in Histopathology Images |  |  |  | 😾 | 
|  | Dual-Stream Multi-Band Fusion Network for Dynamic Functional Connectivity Analysis in Brain Disorder Classification |  |  |  | 😾 | 
|  | EchoingECG: An Electrocardiogram Cross-Modal Model for Echocardiogram Tasks |  |  |  | 😾 | 
|  | Enhancing Soft Tissue Sarcoma Classification by Mitigating Patient-Specific Bias in Whole Slide Images |  |  |  | 😾 | 
|  | Explainable Classifier for Malignant Lymphoma Subtyping via Cell Graph and Image Fusion |  |  |  | 😾 | 
|  | Exposing and Mitigating Calibration Biases and Demographic Unfairness in MLLM Few-Shot In-Context Learning for Medical Image Classification |  |  |  | 😾 | 
|  | FViM: Frequency Vision Mamba for Label-Free Cell Death Pathway Prediction in Lung Cancer Chemotherapy |  |  |  | 😾 | 
|  | Fair-MoE: Medical Fairness-Oriented Mixture of Experts in Vision-Language Models |  |  |  | 😾 | 
|  | Feature Copy-Paste Network for Lung Cancer EGFR Mutation Status Prediction in CT images |  |  |  | 😾 | 
|  | Feature Mixing Approach for Detecting Intraoperative Adverse Events in Laparoscopic Roux-en-Y Gastric Bypass Surgery |  |  |  | 😾 | 
|  | FedWSIDD: Federated Whole Slide Image Classification via Dataset Distillation |  |  |  | 😾 | 
|  | Fine-Grained Rib Fracture Diagnosis with Hyperbolic Embeddings: A Detailed Annotation Framework and Multi-Label Classification Model |  |  |  | 😾 | 
|  | Fine-tuning Vision Language Models with Graph-based Knowledge for Explainable Medical Image Analysis |  |  |  | 😾 | 
|  | Focus on Texture: Rethinking Pre-training in Masked Autoencoders for Medical Image Classification |  |  |  | 😾 | 
|  | FoundBioNet: A Foundation-Based Model for IDH Genotyping of Glioma from Multi-Parametric MRI |  |  |  | 😾 | 
|  | From Pixels to Prognosis: A Multi-Modal Attention-based Framework for Visceral Adipose Tissue Estimation |  |  |  | 😾 | 
|  | GRASP-PsONet: Gradient-based Removal of Spurious Patterns for PsOriasis Severity Classification |  |  |  | 😾 | 
|  | Global and Local Contrastive Learning for Joint Representations from Cardiac MRI and ECG |  |  |  | 😾 | 
|  | Global and Local Vision-Language Alignment for Few-Shot Learning and Few-Shot OOD Detection |  |  |  | 😾 | 
|  | Graph Laplacian Transformer with Progressive Sampling for Prostate Cancer Grading |  |  |  | 😾 | 
|  | Graph-PAVNet: A Graph-Based Learning Framework for Pulmonary Artery and Vein Separation Using Multimodal Feature Sampling |  |  |  | 😾 | 
|  | HARM3-Fusion: Hierarchical Attentional Representation Learning of Multi-Modal, Multi-Temporal, and Multi-Sequence Fusion for Pathological Complete Response Prediction of Head and Neck Squamous Cell Carcinoma |  |  |  | 😾 | 
|  | HalF-SAM: SAM-based Haustral Fold Detection In Colonoscopy with Debris Suppression and Temporal Consistency |  |  |  | 😾 | 
|  | Hard Sample Mining-based Tongue Diagnosis for Fatty Liver Disease Severity Classification |  |  |  | 😾 | 
|  | HiLa: Hierarchical Vision-Language Collaboration for Cancer Survival Prediction |  |  |  | 😾 | 
|  | Hierarchical Corpus-View-Category Refinement for Carotid Plaque Risk Grading in Ultrasound |  |  |  | 😾 | 
|  | Holistic White-light Polyp Classification via Alignment-free Dense Distillation of Auxiliary Optical Chromoendoscopy |  |  |  | 😾 | 
|  | Hybrid-View Attention Network for Clinically Significant Prostate Cancer Classification in Transrectal Ultrasound |  |  |  | 😾 | 
|  | IP-CRR: Information Pursuit for Interpretable Classification of Chest Radiology Reports |  |  |  | 😾 | 
|  | Improving Autism Detection with Multimodal Behavioral Analysis |  |  |  | 😾 | 
|  | Indepth Integration of Multi-granularity Features from Dual-modal for Disease Classification |  |  |  | 😾 | 
|  | Influence of Classification Task and Distribution Shift Type on OOD Detection in Fetal Ultrasound |  |  |  | 😾 | 
|  | Information Bottleneck-based Causal Attention for Multi-label Medical Image Recognition |  |  |  | 😾 | 
|  | Integrate Semantic Radiomics as Prior Evidence into Evidential Deep Learning for Pelvic Lipomatosis Diagnosis |  |  |  | 😾 | 
|  | Knowledge-guided Multi-scale Graph Mamba for Whole Slide Image Classification |  |  |  | 😾 | 
|  | LG-DBGL: Lateralization-Guided Dissociative Brain Graph Learning for Alzheimer’s Disease Identification |  |  |  | 😾 | 
|  | LLM-Powered Cross-Modal Alignment for Explainable Seizure Detection from EEG |  |  |  | 😾 | 
|  | Learning 3D Medical Image Models From Brain Functional Connectivity Network Supervision For Mental Disorder Diagnosis |  |  |  | 😾 | 
|  | Learning Concept-Driven Logical Rules for Interpretable and Generalizable Medical Image Classification |  |  |  | 😾 | 
|  | Learning Contrastive Multimodal Fusion with Improved Modality Dropout for Disease Detection and Prediction |  |  |  | 😾 | 
|  | Learning Explainable Imaging-Genetics Associations Related to a Neurological Disorder |  |  |  | 😾 | 
|  | Lesion-centered vision transformer for stroke outcome prediction from image and clinical data |  |  |  | 😾 | 
|  | Leveraging Diffusion Models for Continual Test-Time Adaptation in Fundus Image Classification |  |  |  | 😾 | 
|  | Location-Guided Automated Lesion Captioning in Whole-body PET/CT Images |  |  |  | 😾 | 
|  | Longitudinal MRI-Clinical Multimodal Fusion for pCR Prediction in Breast Cancer |  |  |  | 😾 | 
|  | Lymph Node Metastasis Classification with Prototype-guided Multiple Instance Aggregation and Heterogeneous Feature Fusion |  |  |  | 😾 | 
|  | MAKE: Multi-Aspect Knowledge-Enhanced Vision-Language Pretraining for Zero-shot Dermatological Assessment |  |  |  | 😾 | 
|  | MELON: Multimodal Mixture-of-Experts with Spectral-Temporal Fusion for Long-Term MObility EstimatioN in Critical Care |  |  |  | 😾 | 
|  | MGG-Net: A Multi-Modal Feature Extraction and Global-Aware Feature Graph-Based Deep Learning Network for MGMT Status Classification in Glioma |  |  |  | 😾 | 
|  | MIBF-Net: Multi-modal Information Balanced Fusion Network for Clinical Diagnosis via Patient Narratives and Lesion Image |  |  |  | 😾 | 
|  | MM-DINOv2: Adapting Foundation Models for Multi-Modal Medical Image Analysis |  |  |  | 😾 | 
|  | MMBNA: Masked Multiview Brain Network Analysis via Disentangling for Alzheimer’s Early Diagnosis with fMRI |  |  |  | 😾 | 
|  | MOC: Meta-Optimized Classifier for Few-Shot Whole Slide Image Classification |  |  |  | 😾 | 
|  | MSGFlowNet: Learning Effective Connectivity Network based on Sparse Generative Flow Network from fMRI and EEG Data |  |  |  | 😾 | 
|  | MSWAL: 3D Multi-class Segmentation of Whole Abdominal Lesions Dataset |  |  |  | 😾 | 
|  | MT-WilmsNet: A Multi-Level Transformer Fusion Network for Wilms’ Tumor Segmentation and Metastasis Prediction |  |  |  | 😾 | 
|  | MambaMER: Adaptive EEG-Guided Multimodal Emotion Recognition with Mamba |  |  |  | 😾 | 
|  | MeDi: Metadata-Guided Diffusion Models for Mitigating Biases in Tumor Classification |  |  |  | 😾 | 
|  | MedSoft-Diffusion: Medical Semantic-Guided Diffusion Model with Soft Mask Conditioning for Vertebral Disease Diagnosis |  |  |  | 😾 | 
|  | Medical-Knowledge Driven Multiple Instance Learning for Classifying Severe Abdominal Anomalies on Prenatal Ultrasound |  |  |  | 😾 | 
|  | Meta-analysis guided multi-task graph transformer network for diagnosis of neurological disease and cognitive deficits |  |  |  | 😾 | 
|  | Metastatic Lymph Node Station Classification in Esophageal Cancer via Prior-guided Supervision and Station-Aware Mixture-of-Experts |  |  |  | 😾 | 
|  | MoMIL: Mixture of Multi-Instance Learners for Modeling Multiple Compound Activities in High Content Imaging |  |  |  | 😾 | 
|  | Multi-Masked Querying Network for Robust Emotion Recognition from Incomplete Multi-Modal Physiological Signals |  |  |  | 😾 | 
|  | Multi-Modal Graph-Based Machine Learning for Predicting Surgical Outcome in Epilepsy Patients |  |  |  | 😾 | 
|  | Multi-Modal Progressive Fusion for ASD Screening Using Smartphone Video |  |  |  | 😾 | 
|  | Multi-Spatial Granger Causality Features Fusion Network for Alzheimer’s Disease Classification |  |  |  | 😾 | 
|  | Multi-modal Knowledge Decomposition based Online Distillation for Biomarker Prediction in Breast Cancer Histopathology |  |  |  | 😾 | 
|  | Multi-modal Representations for Fine-grained Multi-label Critical View of Safety Recognition |  |  |  | 😾 | 
|  | Multi-view Graph Contrastive Learning with Dynamic Self-aware and Cross-sample Topology Augmentation for Brain Disorder Diagnosis |  |  |  | 😾 | 
|  | Multimodal Fusion Network with Distribution-based Tumor-Marker Imputation for Multi-Origin Metastatic Cervical Lymphadenopathy Classification |  |  |  | 😾 | 
|  | Multistage Alignment and Fusion for Multimodal Multiclass Alzheimer’s Disease Diagnosis |  |  |  | 😾 | 
|  | Multiview Feature Fusion and Contrastive Learning for Drug-Target Interaction Prediction |  |  |  | 😾 | 
|  | MvHo-IB: Multi-View Higher-Order Information Bottleneck for Brain Disorder Diagnosis |  |  |  | 😾 | 
|  | NQNN: Noise-aware Quantum Neural Networks for Medical Image Classification |  |  |  | 😾 | 
|  | Neighborhood-Consistent Binary Transformation for Domain-Invariant Chest X-ray Diagnosis |  |  |  | 😾 | 
|  | Neuro-AMS: Neuro-informed Age-aware and Medical Knowledge-integrated Strategy for Diagnosis of Multiple Brain Disorders |  |  |  | 😾 | 
|  | NeuroXVocal: Detection and Explanation of Alzheimer’s Disease through Non-invasive Analysis of Picture-prompted Speech |  |  |  | 😾 | 
|  | Non-Invasive TB Detection using Acoustic and Semantic Features from Cough Sounds |  |  |  | 😾 | 
|  | OpenPath: Open-Set Active Learning for Pathology Image Classification via Pre-trained Vision-Language Models |  |  |  | 😾 | 
|  | PATE: Enhancing Few-Shot Pathological Image Classification via Prompt-Based Text-Image Embedding Adaptation |  |  |  | 😾 | 
|  | PCR-MIL: Phenotype Clustering Reinforced Multiple Instance Learning for Whole Slide Image Classification |  |  |  | 😾 | 
|  | PDF-Net: Prototype-Aware Dynamic Fusion Network for Nasopharyngeal Carcinoma T-staging Classification with Epstein-Barr Virus DNA |  |  |  | 😾 | 
|  | PLUS: Plug-and-Play Enhanced Liver Lesion Diagnosis Model on Non-Contrast CT Scans |  |  |  | 😾 | 
|  | PRGNN: Pyramidal Region Graph Neural Network for Region-Based Brain PET Classification |  |  |  | 😾 | 
|  | PathoPrompt: Cross-Granular Semantic Alignment for Medical Pathology Vision-Language Models |  |  |  | 😾 | 
|  | PedCLIP: A Vision-Language model for Pediatric X-rays with Mixture of Body part Experts |  |  |  | 😾 | 
|  | PerioDet: Large-Scale Panoramic Radiograph Benchmark for Clinical-Oriented Apical Periodontitis Detection |  |  |  | 😾 | 
|  | PolarDETR: Enhancing Interpretability in Multi-modal Methods for Jawbone Lesion Detection in CBCT |  |  |  | 😾 | 
|  | Pose as Clinical Prior: Learning Dual Representations for Scoliosis Screening |  |  |  | 😾 | 
|  | Pretraining on Chronic Lung Inflammatory Disease Datasets to Enhance Indeterminant Lung Cancer Classification using Masked Autoencoders |  |  |  | 😾 | 
|  | Prior-guided Prototype Aggregation Learning for Alzheimer’s Disease Diagnosis |  |  |  | 😾 | 
|  | Privacy Preserving Chest X-ray Classification in Latent Space with Homomorphically Encrypted Neural Inference |  |  |  | 😾 | 
|  | ProTeUS: A Spatio-Temporal Enhanced Ultrasound-Based Framework for Prostate Cancer Detection |  |  |  | 😾 | 
|  | Probabilistic Integration of Renal Cancer Radiology and Pathology Using Graph Neural Networks |  |  |  | 😾 | 
|  | Prototype-Based Multiple Instance Learning for Gigapixel Whole Slide Image Classification |  |  |  | 😾 | 
|  | RIFNet: Bridging Modalities for Accurate and Detailed Ocular Disease Analysis |  |  |  | 😾 | 
|  | RadKAM: Attention-Driven Kolmogorov-Arnold Model for Automatic Radiation-Induced Lymphopenia Prediction by Multimodal Learning |  |  |  | 😾 | 
|  | Radar-Based Imaging for Sign Language Recognition in Medical Communication |  |  |  | 😾 | 
|  | RadioFormer: Integrating Radiologist Inductive Bias for Tumor Classification on Multi-Sequence MR Images |  |  |  | 😾 | 
|  | Rethinking Multi-view Mammogram Representation Learning via Counterfactual Reasoning with Kolmogorov-Arnold Theorem |  |  |  | 😾 | 
|  | Revisiting Masked Image Modeling with Standardized Color Space for Domain Generalized Fundus Photography Classification |  |  |  | 😾 | 
|  | Robust Incomplete-Modality Alignment for Ophthalmic Disease Grading and Diagnosis via Labeled Optimal Transport |  |  |  | 😾 | 
|  | Robust Multimodal Learning for Ophthalmic Disease Grading via Disentangled Representation |  |  |  | 😾 | 
|  | Robust Sleep Stage Prediction from Electroencephalogram with Label Noise Using Multimodal Large Language Models |  |  |  | 😾 | 
|  | SABPI-Net: A Novel Structure-Aware Network for Accurate and Domain-Invariant Retinopathy of Prematurity Diagnosis |  |  |  | 😾 | 
|  | SAMASK-CLTR: A spatial-aware mask guided learning model for benign and malignant tumor classification in ABUS |  |  |  | 😾 | 
|  | SPromptGL: Semantic Prompt Guided Graph Learning for Multi-modal Brain Disease |  |  |  | 😾 | 
|  | SSPNet: Towards Feasible Spatio-Spectral Portraits-Based Deep Learning Framework for Neurodegenerative Disease Multi-Classification |  |  |  | 😾 | 
|  | Self-supervised Normality Learning and Divergence Vector-guided Model Merging for Zero-shot Congenital Heart Disease Detection in Fetal Ultrasound Videos |  |  |  | 😾 | 
|  | SemiVT-Surge: Semi-Supervised Video Transformer for Surgical Phase Recognition |  |  |  | 😾 | 
|  | ShareLink: Neuro-Inspired EEG-based Cross-Subject Emotion Recognition via Shared Bi-hemisphere |  |  |  | 😾 | 
|  | Small Lesions-aware Bidirectional Multimodal Multiscale Fusion Network for Lung Disease Classification |  |  |  | 😾 | 
|  | Solving Medical Multi-Label Domain Adaptation via Wasserstein Adversarial Learning with Class-Level Alignment |  |  |  | 😾 | 
|  | Subtyping Breast Lesions via Generative Augmentation based Long-tailed Recognition in Ultrasound |  |  |  | 😾 | 
|  | T&F-DFC FusionNet: Time&Frequency-Dynamic Functional Connectivity Fusion Network for ADHD Diagnosis in Children based on fNIRS |  |  |  | 😾 | 
|  | T2WI-BCMIC: Non-Fat Saturated T2-Weighted Imaging Dataset for Bladder Cancer Muscle Invasion Classification |  |  |  | 😾 | 
|  | Task-aligned fMRI Generation Model for Brain Disorder Diagnosis |  |  |  | 😾 | 
|  | TemSAM: Temporal-aware Segment Anything Model for Cerebrovascular Segmentation in Digital Subtraction Angiography Sequences |  |  |  | 😾 | 
|  | Temporal Model-Based Federated Active Medical Image Classification |  |  |  | 😾 | 
|  | Temporal Representation Learning of Phenotype Trajectories for pCR Prediction in Breast Cancer |  |  |  | 😾 | 
|  | Time-Lapse Video-Based Embryo Grading via Complementary Spatial-Temporal Pattern Mining |  |  |  | 😾 | 
|  | Toward Medical Deepfake Detection: A Comprehensive Dataset and Novel Method |  |  |  | 😾 | 
|  | Towards Automated Pediatric Dental Development Staging: A Dataset and Model |  |  |  | 😾 | 
|  | Training-free Test-time Improvement for Explainable Medical Image Classification |  |  |  | 😾 | 
|  | Trustworthy Few-Shot Transfer of Medical VLMs through Split Conformal Prediction |  |  |  | 😾 | 
|  | Tumor Microenvironment-Guided Fine-Tuning of Pathology Foundation Models for Esophageal Squamous Cell Carcinoma Immunotherapy Response Prediction |  |  |  | 😾 | 
|  | UltraAD: Fine-Grained Ultrasound Anomaly Classification via Few-Shot CLIP Adaptation |  |  |  | 😾 | 
|  | Uncertainty-Aware Multi-Expert Knowledge Distillation for Imbalanced Disease Grading |  |  |  | 😾 | 
|  | Uncertainty-Aware Multimodal MRI Fusion for HIV-Associated Asymptomatic Neurocognitive Impairment Prediction |  |  |  | 😾 | 
|  | UniCross: Balanced Multimodal Learning for Alzheimer’s Disease Diagnosis by Uni-modal Separation and Metadata-guided Cross-modal Interaction |  |  |  | 😾 | 
|  | VAMPIRE: Uncovering Vessel Directional and Morphological Information from OCTA Images for Cardiovascular Disease Risk Factor Prediction |  |  |  | 😾 | 
|  | VMRA-MaR: An Asymmetry-Aware Temporal Framework for Longitudinal Breast Cancer Risk Prediction |  |  |  | 😾 | 
|  | VT-SNN: Variable Time-step Spiking Neural Network Based on Uncertainty Measure and Its Application in Brain Disease Diagnosis |  |  |  | 😾 | 
|  | Various Attention Mechanism Graph Convolutional Network with Multi-Source Domain Adaptation for Cross-Subject EEG Emotion Recognition |  |  |  | 😾 | 
|  | ViTAL-CT: Vision Transformers for High-Risk Plaque Classification in Coronary CTA |  |  |  | 😾 | 
|  | WSI-Agents: A Collaborative Multi-Agent System for Multi-Modal Whole Slide Image Analysis |  |  |  | 😾 | 
|  | Weighted Stratification in Multi-Label Contrastive Learning for Long-Tailed Medical Image Classification |  |  |  | 😾 | 
|  | X-SiT: Inherently Interpretable Surface Vision Transformers for Dementia Diagnosis |  |  |  | 😾 | 
|  | XFMamba: Cross-Fusion Mamba for Multi-View Medical Image Classification |  |  |  | 😾 | 
</details>

<details>
  <summary><strong>ISBI 2025</strong>（共筛选出 100 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
|  | 3D Multi-Modal Feature Interaction Fusion Network for Early Alzheimer's Disease Diagnosis |  |  |  | 😾 | 
|  | A Dual-Track Framework for Enhancing Age-Related Macular Degeneration Diagnosis Through Embedding and Vision Classification |  |  |  | 😾 | 
|  | A Hierarchical Slice Attention Network for Appendicitis Classification in 3D CT Scans |  |  |  | 😾 | 
|  | A Multi-Modal Fusion Method for 3-Hinge Gyrus Based on Bidirectional Cross-Attention |  |  |  | 😾 | 
|  | A Novel Approach to Recognising Physical Activity Intensity in Adults: Gravity-Based Acceleration Imaging with Vision Transformer |  |  |  | 😾 | 
|  | A Novel Deep Learning Tractography Fiber Clustering Framework for Functionally Consistent White Matter Parcellation Using Multimodal Diffusion MRI and Functional MRI |  |  |  | 😾 | 
|  | A Semi-Supervised Learning Framework with Cross-Magnification Attention for Glioma Mitosis Classification |  |  |  | 😾 | 
|  | ASDC-NET: Anterior Segment Disease Classification Network Based on Slit-Lamp Images |  |  |  | 😾 | 
|  | Active Prompt Tuning Enables GPT-4O to do Efficient Classification of Microscopy Images |  |  |  | 😾 | 
|  | Advancing OCT-Based Retinal Disease Classification with XLSTM: A Framework for Variable-Length Volume Processing |  |  |  | 😾 | 
|  | Age Knowledge Guided CNN-Transformer Network for Alzheimer's Disease Diagnosis |  |  |  | 😾 | 
|  | Anatomy-Guided Multitask Learning for MRI-Based Classification of Placenta Accreta Spectrum and its Subtypes |  |  |  | 😾 | 
|  | Attention-Driven Multi-Sequence Mri Feature Learning and Fusion for Breast Cancer Diagnosis |  |  |  | 😾 | 
|  | Automatic Classification and Explanation of Abnormal EEGs Using Signal Images and Spectrograms |  |  |  | 😾 | 
|  | Beyond Occlusion: In Search for Near Real-Time Explainability of CNN-Based Prostate Cancer Classification |  |  |  | 😾 | 
|  | Brain-Adapter: Enhancing Neurological Disorder Analysis with Adapter-Tuning Multimodal Large Language Models |  |  |  | 😾 | 
|  | Bridging Classification and Segmentation in Osteosarcoma Assessment via Foundation and Discrete Diffusion Models |  |  |  | 😾 | 
|  | CFBM: Cubic Filtered Brain Map Creation Method and Models for Eeg-Based Emotion Recognition |  |  |  | 😾 | 
|  | CINEPRO: Robust Training of Foundation Models for Cancer Detection in Prostate Ultrasound Cineloops |  |  |  | 😾 | 
|  | CSF-NET: Cross-Modal Spatiotemporal Fusion Network for Pulmonary Nodule Malignancy Predicting |  |  |  | 😾 | 
|  | CascadePLS-ViT: Cascade with Patch-Level Self-Supervised Vision Transformers for Breast Cancer Classification in Mammography |  |  |  | 😾 | 
|  | Causal Modeling of FMRI Time-Series for Interpretable Autism Spectrum Disorder Classification |  |  |  | 😾 | 
|  | Cell Image Classification with Polygon-Guided Extraction for Glioma Mitosis Detection |  |  |  | 😾 | 
|  | Cervical Cancer Lesion Grading Based on Clinical Diagnostic Criteria and Multi-View Approach Using Intraoperative Colposcopic Images |  |  |  | 😾 | 
|  | Class Fusion of Support Vector Machines with Deep Learning Features for Oral Cancer Histopathology Classification |  |  |  | 😾 | 
|  | Classification of Oral Potentially Malignant Disorders Using Multimodal Feature Integration |  |  |  | 😾 | 
|  | Classification of Surgeons' Gender from Videos of the Surgical Field |  |  |  | 😾 | 
|  | Combining 4D Hyperspectral Imaging With CNN for Nerve and Ligament Differentiation |  |  |  | 😾 | 
|  | Comparative Analysis of Unsupervised and Supervised Autoencoders for Nuclei Classification in Clear Cell Renal Cell Carcinoma Images |  |  |  | 😾 | 
|  | Conditional Convolution of Clinical Data Embeddings for Multimodal Prostate Cancer Classification |  |  |  | 😾 | 
|  | Conditional Visuo-Textual Prompt Learning for Medical Image Analysis |  |  |  | 😾 | 
|  | Connectome-GTC: A Unified Framework for Brain Functional and Structural Connectomes Generation, Translation, and Classification |  |  |  | 😾 | 
|  | Deep Learning-Driven Automated Aortic Valve Calcification Assessment in CT Calcium Score Screening Images |  |  |  | 😾 | 
|  | Deep Neural Network for Lung Adenocarcinoma Subtype from Multimodal Fusion of Imaging and Clinical Data |  |  |  | 😾 | 
|  | Diagnosis with Nanoscale Protein Distributions: Single-Molecule Fluorescence Localization Microscopy and Attention-Based Learning |  |  |  | 😾 | 
|  | Drop and Align: Fusing Pathology and Genomics via Graph Learning for Cancer Survival Prediction |  |  |  | 😾 | 
|  | Dual Invariance Self-Training for Reliable Semi-Supervised Surgical Phase Recognition |  |  |  | 😾 | 
|  | Dual Multi-Atlas Representation Alignment for Brain Disorder Diagnosis Using Morphological Connectome |  |  |  | 😾 | 
|  | Efficient 4D fMRI ASD Classification using Spatial-Temporal-Omics-Based Learning Framework |  |  |  | 😾 | 
|  | Ensemble of Foundation Models for Pap Smear Cell Analysis |  |  |  | 😾 | 
|  | Exploring Foundation Models Fine-Tuning for Cytology Classification |  |  |  | 😾 | 
|  | F2UE-NET: An Uncertainty-Driven Dynamic Expert Network for Long-Tail Retinal Disease Classification |  |  |  | 😾 | 
|  | Fairness Analysis of Clip-Based Foundation Models for X-Ray Image Classification |  |  |  | 😾 | 
|  | Functional-Positional Context Embedding in Graph Neural Networks for Improved Parkinson's Disease Diagnosis |  |  |  | 😾 | 
|  | Fusion of Dwi Image and Clinical Variables for Stroke Outcome Prediction Using Tabular Transformer |  |  |  | 😾 | 
|  | Generalized Cross-Domain Multi-Label Few-Shot Learning for Chest X-Rays |  |  |  | 😾 | 
|  | Generating Customized Prompts for Zero-Shot Rare Event Medical Image Classification Using LLM |  |  |  | 😾 | 
|  | Genetics Encoded Joint Embedding of Multimodal Connectomes with Explainable Graph Neural Network for Schizophrenia Classification |  |  |  | 😾 | 
|  | Guiding the Classification of Hepatocellular Carcinoma on 3D CT-Scans Using Deep and Handcrafted Radiological Features |  |  |  | 😾 | 
|  | HER2 Status Prediction in Breast Cancer: A Weak Supervision and Contrastive Learning Approach on H&E Stains |  |  |  | 😾 | 
|  | Hybrid DL Classification Model Based on CNN and Transformer for Pap Smear Cells |  |  |  | 😾 | 
|  | ISGM-DR: Important Slice-Guided Model for Diabetic Retinopathy Diagnosis Using 3D Oct Images |  |  |  | 😾 | 
|  | ITCFN: Incomplete Triple-Modal Co-Attention Fusion Network for Mild Cognitive Impairment Conversion Prediction |  |  |  | 😾 | 
|  | Improved Brain Disorder Classification in RS-FMRI via Inter-Individual Variability Characterization: A Self-Supervised Learning Approach |  |  |  | 😾 | 
|  | Knowledge-Aware Multi-Source Domain Adaptation Network for Osteoporosis Diagnosis from Ultrasonic Radio Frequency Signals |  |  |  | 😾 | 
|  | Large-Scale Radiomics Analysis for Prostate Cancer Detection Harnessing Machine and Deep Learning Models |  |  |  | 😾 | 
|  | Longmambattn: A Novel Architecture for Enhanced Breast Cancer Risk Prediction Using Variable-Length Longitudinal Mammograms |  |  |  | 😾 | 
|  | MUM: Enhancing Medical Diagnostics Through Unpaired Multimodal Data Integration |  |  |  | 😾 | 
|  | Multi-Distribution Mammogram Classification: Leveraging Clip for Enhanced Generalization Across Diverse Datasets |  |  |  | 😾 | 
|  | Multi-Modal Fusion Network Integrating Imaging and Clinical Tabular Data for Alzheimer's Disease Classification |  |  |  | 😾 | 
|  | Multi-Modal Imaging Genomics Transformer: Attentive Integration of Imaging with Genomic Biomarkers for Schizophrenia Classification |  |  |  | 😾 | 
|  | Multi-Modal Integration for Predicting Biochemical Recurrence in Prostate Cancer |  |  |  | 😾 | 
|  | Multi-Scale Time-Frequency Fusion Intelligent Diagnosis Model for Children with ADHD Based on fNIRS |  |  |  | 😾 | 
|  | Multi-VentNet: Multimodal Fusion Framework for Predicting Need for Mechanical Ventilation |  |  |  | 😾 | 
|  | Multicenter Radiomic Feature-Based Prediction of PD-L1 Status in Lung Cancer Using Multimodal PET/CT |  |  |  | 😾 | 
|  | Multimodal Learning for Enhanced Detection in Oral Cancer Screening |  |  |  | 😾 | 
|  | Nnmamba: 3D Biomedical Image Segmentation, Classification and Landmark Detection with State Space Model |  |  |  | 😾 | 
|  | Non-Invasive Molecular Classification of Gliomas Using Explainable ai and Radiomic Features from Multi-Sequence MRI |  |  |  | 😾 | 
|  | OCT-Trans: A Novel Transformer Backbone with Multimodal Feature Extraction in OCT-Based Retinal Disease Classification |  |  |  | 😾 | 
|  | Optimizing Cell Recognition in Sarcoma Multiplex Immunohistochemistry Images via Pre-Training Strategies |  |  |  | 😾 | 
|  | Overall Survival Prediction of Brain Tumor Patients with Multimodal MRI using Swin Unetr |  |  |  | 😾 | 
|  | PI-RADS Inspired Architecture: A New Deep Learning Approach to Detect Clinically Significant Prostate Cancer on MRI |  |  |  | 😾 | 
|  | PSO-NET: Development of an Automated Psoriasis Assessment System Using Attention-Based Interpretable Deep Neural Networks |  |  |  | 😾 | 
|  | Predicting Axillary Lymph Node Metastasis in Breast Cancer by Integrating Dual-ROI Features from Multi-Parametric MRI |  |  |  | 😾 | 
|  | Predicting Response to Neoadjuvant Chemotherapy using Multi-Modal MRI Radiomics and Machine Learning Approaches |  |  |  | 😾 | 
|  | Predicting Ulcer in H&E Images of Inflammatory Bowel Disease Using Domain-Knowledge-Driven Graph Neural Network |  |  |  | 😾 | 
|  | Pulmofusion: Advancing Pulmonary Health with Efficient Multi-Modal Fusion |  |  |  | 😾 | 
|  | Quantitative Gait Analysis from Single RGB Videos Using a Dual-Input Transformer-Based Network |  |  |  | 😾 | 
|  | RIEMAE: Riemannian Masked Autoencoder for Classifying Malignant Prostate Cancer Patterns |  |  |  | 😾 | 
|  | RVPD: An Automated System for Calculating the Tortuosity and Bifurcation Angles of Retinal Vessels to Predict Diseases |  |  |  | 😾 | 
|  | Resection Margin Prediction in Pancreatic Ductal Adenocarcinoma: A Multi-Modal Machine Learning Approach Integrating Clinical and Computed Tomography Radiomics Data |  |  |  | 😾 | 
|  | SCONet: Convolutional Occupancy Networks for Multi-Organ Segmentation |  |  |  | 😾 | 
|  | SFECNet: Fusion of Structural, Functional and Effective Connectivity Network for Epilepsy Classification |  |  |  | 😾 | 
|  | Self-Supervised Graph Transformer with Contrastive Learning for Brain Connectivity Analysis Towards Improving Autism Detection |  |  |  | 😾 | 
|  | Self-Supervised Learning for Multi-Category Endoscopy Classification and Data Quality Evaluation Using Masked Autoencoders |  |  |  | 😾 | 
|  | Self-supervised Prototype Learning for Spatio-Temporal Enhanced Ultrasound-based Prostate Cancer Detection |  |  |  | 😾 | 
|  | Semi-Supervised Multimodal Multi-Instance Learning for Aortic Stenosis Diagnosis |  |  |  | 😾 | 
|  | Sequential Spatial-Temporal Network for Interpretable Automatic Ultrasonic Assessment of Fetal Head During Labor |  |  |  | 😾 | 
|  | Shoulder Rotator Cuff Tear Detection from Ultrasound Videos Using Deep Reinforcement Learning |  |  |  | 😾 | 
|  | Slide-Level Prompt Learning with Vision Language Models for Few-Shot Multiple Instance Learning in Histopathology |  |  |  | 😾 | 
|  | Spatiotemporal Learning with Context-Aware Video Tubelets for Ultrasound Video Analysis |  |  |  | 😾 | 
|  | TMI-CLNet: Triple-Modal Interaction Network for Chronic Liver Disease Prognosis from Imaging, Clinical, and Radiomic Data Fusion |  |  |  | 😾 | 
|  | TX-LLaVA: Large Language and Vision Assistant for Temporal Changes in Chest X-Rays |  |  |  | 😾 | 
|  | Temporal Feature Weaving for Neonatal Echocardiographic Viewpoint Video Classification |  |  |  | 😾 | 
|  | Towards Improved Cervical Cancer Screening: Vision Transformer-Based Classification and Interpretability |  |  |  | 😾 | 
|  | Towards Multi-Label Open Set Diagnosis for Medical Images |  |  |  | 😾 | 
|  | Train for Stain: Adapting for Diverse H&E Staining Profiles Across Centers in Classification of Mitotic Figures (Glioma-MDC 2025) |  |  |  | 😾 | 
|  | Ultrasound Image Synthesis Using Generative AI for Lung Consolidation Detection |  |  |  | 😾 | 
|  | Unveiling the Capabilities of Latent Diffusion Models for Classification of Lung Diseases in Chest X-Rays |  |  |  | 😾 | 
|  | Visual Prompting and Adaptation of Vision Language Models for Tumor Classification in Breast Ultrasound |  |  |  | 😾 | 
</details>

<details>
  <summary><strong>ICML 2025</strong>（共筛选出 19 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 859NdHQv0Z | [OmiAD: One-Step Adaptive Masked Diffusion Model for Multi-class Anomaly Detection via Adversarial Distillation](https://openreview.net/pdf?id=859NdHQv0Z) | Yaoxuan Feng | Poster |  | 😾 | 
| CKQI3Nksxm | [Learning from True-False Labels via Multi-modal Prompt Retrieving](https://openreview.net/pdf?id=CKQI3Nksxm) | Zhongnian Li | Poster | 0 | 😾 | 
| EuJaF5QsMP | [$\texttt{I$^2$MoE}$: Interpretable Multimodal Interaction-aware Mixture-of-Experts](https://openreview.net/pdf?id=EuJaF5QsMP) | Jiayi Xin | Poster |  | 😾 | 
| F7BOaYmWl7 | [How Do Images Align and Complement LiDAR? Towards a Harmonized Multi-modal 3D Panoptic Segmentation](https://openreview.net/pdf?id=F7BOaYmWl7) | Yining Pan | Poster |  | 😾 | 
| MzQed6QEmS | [Modalities Contribute Unequally: Enhancing Medical Multi-modal Learning through Adaptive Modality Token Re-balancing](https://openreview.net/pdf?id=MzQed6QEmS) | Jie Peng | Poster |  | 😾 | 
| QIL44dSUPo | [Handling Imbalanced Pseudolabels for Vision-Language Models with Concept Alignment and Confusion-Aware Calibrated Margin](https://openreview.net/pdf?id=QIL44dSUPo) | Yuchen Wang | Poster | 0 | 😾 | 
| RLu1QIPiVr | [H-Tuning: Toward Low-Cost and Efficient ECG-based Cardiovascular Disease Detection with Pre-Trained Models](https://openreview.net/pdf?id=RLu1QIPiVr) | Rushuang Zhou | Poster |  | 😾 | 
| RbsfEuYNTA | [Learning Compact Semantic Information for Incomplete Multi-View Missing Multi-Label Classification](https://openreview.net/pdf?id=RbsfEuYNTA) | Jie Wen | Poster |  | 😾 | 
| XCLZgbm99O | [FOCoOp: Enhancing Out-of-Distribution Robustness in Federated Prompt Learning for Vision-Language Models](https://openreview.net/pdf?id=XCLZgbm99O) | Xinting Liao | Poster |  | 😾 | 
| Y8lfuSoqQz | [OV-MER: Towards Open-Vocabulary Multimodal Emotion Recognition](https://openreview.net/pdf?id=Y8lfuSoqQz) | Zheng Lian | Poster |  | 😾 | 
| ZkybWzxvw9 | [MoHAVE: Mixture of Hierarchical Audio-Visual Experts for Robust Speech Recognition](https://openreview.net/pdf?id=ZkybWzxvw9) | Sungnyun Kim | Poster | 2 | 😾 | 
| dewR2augg2 | [FlexiReID: Adaptive Mixture of Expert for Multi-Modal Person Re-Identification](https://openreview.net/pdf?id=dewR2augg2) | Zhen Sun | Poster |  | 😾 | 
| e6vlZdLlIK | [MMedPO: Aligning Medical Vision-Language Models with Clinical-Aware Multimodal Preference Optimization](https://openreview.net/pdf?id=e6vlZdLlIK) | Kangyu Zhu | Poster | 3 | 😾 | 
| fRoRsmPHUM | [LEMoN: Label Error Detection using Multimodal Neighbors](https://openreview.net/pdf?id=fRoRsmPHUM) | Haoran Zhang | Poster | 0 | 😾 | 
| fjXcRSfyIV | [BDC-CLIP: Brownian Distance Covariance for Adapting CLIP to Action Recognition](https://openreview.net/pdf?id=fjXcRSfyIV) | Fei Long | Poster |  | 😾 | 
| muZEsBQzlA | [Tensorized Multi-View Multi-Label Classification via Laplace Tensor Rank](https://openreview.net/pdf?id=muZEsBQzlA) | Qiyu Zhong | Poster |  | 😾 | 
| vDB2oX3Wl3 | [Unlocking the Capabilities of Large Vision-Language Models for Generalizable and Explainable Deepfake Detection](https://openreview.net/pdf?id=vDB2oX3Wl3) | Peipeng Yu | Poster |  | 😾 | 
| xAhUoyb5eU | [Ex-VAD: Explainable Fine-grained Video Anomaly Detection Based on Visual-Language Models](https://openreview.net/pdf?id=xAhUoyb5eU) | Chao Huang | Poster |  | 😾 | 
| xmbdACI0xu | [AffectGPT: A New Dataset, Model, and Benchmark for Emotion Understanding with Multimodal Large Language Models](https://openreview.net/pdf?id=xmbdACI0xu) | Zheng Lian | Oral | 3 | 😾 | 
</details>

<details>
  <summary><strong>ICLR 2025</strong>（共筛选出 16 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 10JOlFIPjt | [In vivo cell-type and brain region classification via multimodal contrastive learning](https://openreview.net/pdf?id=10JOlFIPjt) | Han Yu | Spotlight | 1 | 😾 | 
| A1HhtITVEi | [CheapNet: Cross-attention on Hierarchical representations for Efficient protein-ligand binding Affinity Prediction](https://openreview.net/pdf?id=A1HhtITVEi) | Hyukjun Lim | Poster | 0 | 😾 | 
| AD5yx2xq8R | [XAIguiFormer: explainable artificial intelligence guided transformer for brain disorder identification](https://openreview.net/pdf?id=AD5yx2xq8R) | Hanning Guo | Poster | 0 | 😾 | 
| B5VEi5d3p2 | [SleepSMC: Ubiquitous Sleep Staging via Supervised Multimodal Coordination](https://openreview.net/pdf?id=B5VEi5d3p2) | Shuo Ma | Poster | 0 | 😾 | 
| D6zn6ozJs7 | [MMFakeBench: A Mixed-Source Multimodal Misinformation Detection Benchmark for LVLMs](https://openreview.net/pdf?id=D6zn6ozJs7) | Xuannan Liu | Poster | 0 | 😾 | 
| Ev4iw23gdI | [EMMA: Empowering Multi-modal Mamba with Structural and Hierarchical Alignment](https://openreview.net/pdf?id=Ev4iw23gdI) | Yifei Xing | Poster | 0 | 😾 | 
| M3kBtqpys5 | [Trusted Multi-View Classification via Evolutionary Multi-View Fusion](https://openreview.net/pdf?id=M3kBtqpys5) | Xinyan Liang | Poster | 0 | 😾 | 
| N5YTixK4F1 | [IDA-VLM: Towards Movie Understanding via ID-Aware Large Vision-Language Model](https://openreview.net/pdf?id=N5YTixK4F1) | Yatai Ji | Poster | 3 | 😾 | 
| NJxCpMt0sf | [Dynamic Modeling of Patients, Modalities and Tasks via Multi-modal Multi-task Mixture of Experts](https://openreview.net/pdf?id=NJxCpMt0sf) | Chenwei Wu | Poster | 0 | 😾 | 
| fv9XU7CyN2 | [CL-MFAP: A Contrastive Learning-Based Multimodal Foundation Model for Molecular Property Prediction and Antibiotic Screening](https://openreview.net/pdf?id=fv9XU7CyN2) | Gen Zhou | Poster | 0 | 😾 | 
| g1fkhbhHjL | [Black Sheep in the Herd: Playing with Spuriously Correlated Attributes for Vision-Language Recognition](https://openreview.net/pdf?id=g1fkhbhHjL) | Xinyu Tian | Poster | 0 | 😾 | 
| jXvwJ51vcK | [Multimodality Helps Few-shot 3D Point Cloud Semantic Segmentation](https://openreview.net/pdf?id=jXvwJ51vcK) | Zhaochong An | Spotlight | 3 | 😾 | 
| jZsN9zo8Qi | [Learning Interleaved Image-Text Comprehension in Vision-Language Large Models](https://openreview.net/pdf?id=jZsN9zo8Qi) | Chenyu Zhou | Poster | 0 | 😾 | 
| nYpPAT4L3D | [Large-scale and Fine-grained Vision-language Pre-training for Enhanced CT Image Understanding](https://openreview.net/pdf?id=nYpPAT4L3D) | Zhongyi Shui | Spotlight | 1 | 😾 | 
| p3NKpom1VL | [Analyzing and Boosting the Power of Fine-Grained Visual Recognition for Multi-modal Large Language Models](https://openreview.net/pdf?id=p3NKpom1VL) | Hulingxiao He | Poster | 4 | 😾 | 
| pAQzEY7M03 | [FakeShield: Explainable Image Forgery Detection and Localization via Multi-modal Large Language Models](https://openreview.net/pdf?id=pAQzEY7M03) | Zhipei Xu | Poster | 20 | 😾 | 
</details>

<details>
  <summary><strong>ICCV 2025</strong>（共筛选出 41 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 26 | [Feature Decomposition-Recomposition in Large Vision-Language Model for Few-Shot Class-Incremental Learning](https://iccv.thecvf.com/virtual/2025/poster/26) | Zongyao Xue | Poster |  | 😾 | 
| 91 | [Bridging the Skeleton-Text Modality Gap: Diffusion-Powered Modality Alignment for Zero-shot Skeleton-based Action Recognition](2411.10745) | Jeonghyeok Do | Poster |  | 😾 | 
| 157 | [Vision-Language Neural Graph Featurization for Extracting Retinal Lesions](https://openaccess.thecvf.com/content/ICCV2025/papers/Hassan_Vision-Language_Neural_Graph_Featurization_for_Extracting_Retinal_Lesions_ICCV_2025_paper.pdf) | Taimur Hassan | Poster |  | 😾 | 
| 214 | [FedMVP: Federated Multimodal Visual Prompt Tuning for Vision-Language Models](2504.20860) | Mainak Singha | Poster |  | 😾 | 
| 220 | [Intra-modal and Cross-modal Synchronization for Audio-visual Deepfake Detection and Temporal Localization](https://openaccess.thecvf.com/content/ICCV2025/papers/Anshul_Intra-modal_and_Cross-modal_Synchronization_for_Audio-visual_Deepfake_Detection_and_Temporal_ICCV_2025_paper.pdf) | Ashutosh Anshul | Poster |  | 😾 | 
| 277 | [Cross-View Isolated Sign Language Recognition via View Synthesis and Feature Disentanglement](https://openaccess.thecvf.com/content/ICCV2025/papers/Shen_Cross-View_Isolated_Sign_Language_Recognition_via_View_Synthesis_and_Feature_ICCV_2025_paper.pdf) | Xin Shen | Poster |  | 😾 | 
| 327 | [ModalTune: Fine-Tuning Slide-Level Foundation Models with Multi-Modal Information for Multi-task Learning in Digital Pathology](2503.17564) | Vishwesh Ramanathan | Poster |  | 😾 | 
| 559 | [MUG: Pseudo Labeling Augmented Audio-Visual Mamba Network for Audio-Visual Video Parsing](2507.01384) | Langyu Wang | Poster |  | 😾 | 
| 597 | [Enhancing Few-Shot Vision-Language Classification with Large Multimodal Model Features](2412.00142) | Chancharik Mitra | Poster |  | 😾 | 
| 625 | [LLaFEA: Frame-Event Complementary Fusion for Fine-Grained Spatiotemporal Understanding in LMMs](2503.06934) | Hanyu Zhou | Poster |  | 😾 | 
| 940 | [Debiased Curriculum Adaptation for Safe Transfer Learning in Chest X-ray Classification](https://openaccess.thecvf.com/content/ICCV2025/papers/Liu_Debiased_Curriculum_Adaptation_for_Safe_Transfer_Learning_in_Chest_X-ray_ICCV_2025_paper.pdf) | Mingyang Liu | Poster |  | 😾 | 
| 996 | [Seeing 3D Through 2D Lenses: 3D Few-Shot Class-Incremental Learning via Cross-Modal Geometric Rectification](https://iccv.thecvf.com/virtual/2025/poster/996) | Tuo Xiang | Poster |  | 😾 | 
| 1177 | [Multi-Cache Enhanced Prototype Learning for Test-Time Generalization of Vision-Language Models](2508.01225) | Xinyu Chen | Poster |  | 😾 | 
| 1238 | [Wave-MambaAD: Wavelet-driven State Space Model for Multi-class Unsupervised Anomaly Detection](https://openaccess.thecvf.com/content/ICCV2025/papers/Zhang_Wave-MambaAD_Wavelet-driven_State_Space_Model_for_Multi-class_Unsupervised_Anomaly_Detection_ICCV_2025_paper.pdf) | Qiao Zhang | Poster |  | 😾 | 
| 1273 | [Causal Disentanglement and Cross-Modal Alignment for Enhanced Few-Shot Learning](2508.03102) | Tianjiao Jiang | Poster |  | 😾 | 
| 1289 | [Griffon v2: Advancing Multimodal Perception with High-Resolution Scaling and Visual-Language Co-Referring](2403.09333) | Yufei Zhan | Poster |  | 😾 | 
| 1325 | [FedVLA: Federated Vision-Language-Action Learning with Dual Gating Mixture-of-Experts for Robotic Manipulation](2508.02190) | Cui Miao | Poster |  | 😾 | 
| 1531 | [MaTVLM: Hybrid Mamba-Transformer for Efficient Vision-Language Modeling](2503.13440) | Yingyue Li | Poster |  | 😾 | 
| 1554 | [GMMamba: Group Masking Mamba for Whole Slide Image Classification](https://openaccess.thecvf.com/content/ICCV2025/papers/Zheng_GMMamba_Group_Masking_Mamba_for_Whole_Slide_Image_Classification_ICCV_2025_paper.pdf) | Tingting Zheng | Poster |  | 😾 | 
| 1557 | [ReMP-AD: Retrieval-enhanced Multi-modal Prompt Fusion for Few-Shot Industrial Visual Anomaly Detection](https://openaccess.thecvf.com/content/ICCV2025/papers/Ma_ReMP-AD_Retrieval-enhanced_Multi-modal_Prompt_Fusion_for_Few-Shot_Industrial_Visual_Anomaly_ICCV_2025_paper.pdf) | Hongchi Ma | Poster |  | 😾 | 
| 1573 | [DocThinker: Explainable Multimodal Large Language Models with Rule-based Reinforcement Learning for Document Understanding](2508.08589) | Wenwen Yu | Poster |  | 😾 | 
| 1678 | [MaskSAM: Auto-prompt SAM with Mask Classification for Volumetric Medical Image Segmentation](https://openaccess.thecvf.com/content/ICCV2025/papers/Xie_MaskSAM_Auto-prompt_SAM_with_Mask_Classification_for_Volumetric_Medical_Image_ICCV_2025_paper.pdf) | Bin Xie | Poster |  | 😾 | 
| 1688 | [Hierarchical Cross-modal Prompt Learning for Vision-Language Models](2507.14976) | Hao Zheng | Poster |  | 😾 | 
| 1722 | [DeSPITE: Exploring Contrastive Deep Skeleton-Pointcloud-IMU-Text Embeddings for Advanced Point Cloud Human Activity Understanding](https://openaccess.thecvf.com/content/ICCV2025/papers/Kreutz_DeSPITE_Exploring_Contrastive_Deep_Skeleton-Pointcloud-IMU-Text_Embeddings_for_Advanced_Point_Cloud_ICCV_2025_paper.pdf) | Thomas Kreutz | Poster |  | 😾 | 
| 1861 | [External Knowledge Injection for CLIP-Based Class-Incremental Learning](2503.08510) | Da-Wei Zhou | Poster |  | 😾 | 
| 1906 | [Federated Prompt-Tuning with Heterogeneous and Incomplete Multimodal Client Data](https://openaccess.thecvf.com/content/ICCV2025/papers/Phung_Federated_Prompt-Tuning_with_Heterogeneous_and_Incomplete_Multimodal_Client_Data_ICCV_2025_paper.pdf) | Thu Hang Phung | Poster |  | 😾 | 
| 1929 | [Dynamic Multimodal Prototype Learning in Vision-Language Models](2507.03657) | Xingyu Zhu | Poster |  | 😾 | 
| 2083 | [AVAM: a Universal Training-free Adaptive Visual Anchoring Embedded into Multimodal Large Language Model for Multi-image Question Answering](https://openaccess.thecvf.com/content/ICCV2025/papers/Zeng_AVAM_a_Universal_Training-free_Adaptive_Visual_Anchoring_Embedded_into_Multimodal_ICCV_2025_paper.pdf) | Kang Zeng | Poster |  | 😾 | 
| 2085 | [FDPT: Federated Discrete Prompt Tuning for Black-Box Visual-Language Models](https://openaccess.thecvf.com/content/ICCV2025/papers/Wu_FDPT_Federated_Discrete_Prompt_Tuning_for_Black-Box_Visual-Language_Models_ICCV_2025_paper.pdf) | Jiaqi Wu | Poster |  | 😾 | 
| 2135 | [AMDANet: Attention-Driven Multi-Perspective Discrepancy Alignment for RGB-Infrared Image Fusion and Segmentation](https://openaccess.thecvf.com/content/ICCV2025/papers/Zhong_AMDANet_Attention-Driven_Multi-Perspective_Discrepancy_Alignment_for_RGB-Infrared_Image_Fusion_and_ICCV_2025_paper.pdf) | Haifeng Zhong | Poster |  | 😾 | 
| 2166 | [CMAD: Correlation-Aware and Modalities-Aware Distillation for Multimodal Sentiment Analysis with Missing Modalities](https://openaccess.thecvf.com/content/ICCV2025/papers/Zhuang_CMAD_Correlation-Aware_and_Modalities-Aware_Distillation_for_Multimodal_Sentiment_Analysis_with_ICCV_2025_paper.pdf) | Yan Zhuang | Poster |  | 😾 | 
| 2333 | [MobileViCLIP: An Efficient Video-Text Model for Mobile Devices](2508.07312) | Min Yang | Poster |  | 😾 | 
| 2346 | [SceneSplat: Gaussian Splatting-based Scene Understanding with Vision-Language Pretraining](2503.18052) | Yue Li | Poster |  | 😾 | 
| 2351 | [Mamba-3VL: Taming State Space Model for 3D Vision Language Learning](https://openaccess.thecvf.com/content/ICCV2025/papers/Wang_Mamba-3VL_Taming_State_Space_Model_for_3D_Vision_Language_Learning_ICCV_2025_paper.pdf) | Yuan Wang | Poster |  | 😾 | 
| 2413 | [MPBR: Multimodal Progressive Bidirectional Reasoning for Open-Set Fine-Grained Recognition](https://openaccess.thecvf.com/content/ICCV2025/papers/Tan_MPBR_Multimodal_Progressive_Bidirectional_Reasoning_for_Open-Set_Fine-Grained_Recognition_ICCV_2025_paper.pdf) | Junfu Tan | Poster |  | 😾 | 
| 2416 | [Synergistic Prompting for Robust Visual Recognition with Missing Modalities](2507.07802) | Zhihui Zhang | Poster |  | 😾 | 
| 2446 | [Multimodal Prompt Alignment for Facial Expression Recognition](2506.21017) | Fuyan Ma | Poster |  | 😾 | 
| 2490 | [Flow-MIL: Constructing Highly-expressive Latent Feature Space For Whole Slide Image Classification Using Normalizing Flow](https://openaccess.thecvf.com/content/ICCV2025/papers/Ma_Flow-MIL_Constructing_Highly-expressive_Latent_Feature_Space_For_Whole_Slide_Image_ICCV_2025_paper.pdf) | Yingfan MA | Poster |  | 😾 | 
| 2507 | [FED-PsyAU: Privacy-Preserving Micro-Expression Recognition via Psychological AU Coordination and Dynamic Facial Motion Modeling](https://openaccess.thecvf.com/content/ICCV2025/papers/Li_FED-PsyAU_Privacy-Preserving_Micro-Expression_Recognition_via_Psychological_AU_Coordination_and_Dynamic_ICCV_2025_paper.pdf) | Jingting Li | Poster |  | 😾 | 
| 2569 | [Exploiting Frequency Dynamics for Enhanced Multimodal Event-based Action Recognition](https://iccv.thecvf.com/virtual/2025/poster/2569) | Meiqi Cao | Poster |  | 😾 | 
| 2596 | [SAMPLE: Semantic Alignment through Temporal-Adaptive Multimodal Prompt Learning for Event-Based Open-Vocabulary Action Recognition](https://openaccess.thecvf.com/content/ICCV2025/papers/Wang_SAMPLE_Semantic_Alignment_through_Temporal-Adaptive_Multimodal_Prompt_Learning_for_Event-Based_ICCV_2025_paper.pdf) | Jing Wang | Poster |  | 😾 | 
</details>

<details>
  <summary><strong>CVPR 2025</strong>（共筛选出 43 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 32427 | [SIDA: Social Media Image Deepfake Detection, Localization and Explanation with Large Multimodal Model](2412.04292) | Zhenglin Huang | Poster | 10 | 😾 | 
| 32434 | [M3amba: Memory Mamba is All You Need for Whole Slide Image Classification](https://openaccess.thecvf.com/content/CVPR2025/papers/Zheng_M3amba_Memory_Mamba_is_All_You_Need_for_Whole_Slide_CVPR_2025_paper.pdf) | Tingting Zheng | Poster | 0 | 😾 | 
| 32445 | [SAM2-LOVE: Segment Anything Model 2 in Language-aided Audio-Visual Scenes](https://openaccess.thecvf.com/content/CVPR2025/papers/Wang_SAM2-LOVE_Segment_Anything_Model_2_in_Language-aided_Audio-Visual_Scenes_CVPR_2025_paper.pdf) | Yuji Wang | Poster | 0 | 😾 | 
| 32494 | [CLIP-driven Coarse-to-fine Semantic Guidance for Fine-grained Open-set Semi-supervised Learning](https://openaccess.thecvf.com/content/CVPR2025/papers/Li_CLIP-driven_Coarse-to-fine_Semantic_Guidance_for_Fine-grained_Open-set_Semi-supervised_Learning_CVPR_2025_paper.pdf) | Xiaokun Li | Poster | 0 | [Code😻](https://github.com/LxxxxK/CFSG-CLIP) | 
| 32496 | [SeqAfford: Sequential 3D Affordance Reasoning via Multimodal Large Language Model](2412.01550) | Chunlin Yu | Poster | 3 | 😾 | 
| 32653 | [CLOC: Contrastive Learning for Ordinal Classification with Multi-Margin N-pair Loss](https://openaccess.thecvf.com/content/CVPR2025/papers/Pitawela_CLOC_Contrastive_Learning_for_Ordinal_Classification_with_Multi-Margin_N-pair_Loss_CVPR_2025_paper.pdf) | Dileepa Pitawela | Poster | 0 | [Code😻](https://github.com/dpitawela/CLOC) | 
| 32654 | [Occlusion-aware Text-Image-Point Cloud Pretraining for Open-World 3D Object Recognition](2502.10674) | Khanh Nguyen | Poster | 0 | [Code😻](ndkhanh360.github.io/project-occtip) | 
| 32675 | [HistoFS: Non-IID Histopathologic Whole Slide Image Classification via Federated Style Transfer with RoI-Preserving](https://openaccess.thecvf.com/content/CVPR2025/papers/Raswa_HistoFS_Non-IID_Histopathologic_Whole_Slide_Image_Classification_via_Federated_Style_CVPR_2025_paper.pdf) | Farchan Hakim Raswa | Poster | 0 | 😾 | 
| 32709 | [Rethinking Vision-Language Model in Face Forensics: Multi-Modal Interpretable Forged Face Detector](2503.20188) | Xiao Guo | Poster | 3 | 😾 | 
| 32718 | [Uncertain Multimodal Intention and Emotion Understanding in the Wild](https://openaccess.thecvf.com/content/CVPR2025/papers/Yang_Uncertain_Multimodal_Intention_and_Emotion_Understanding_in_the_Wild_CVPR_2025_paper.pdf) | Qu Yang | Poster | 2 | [Code😻](https://github.com/yan9qu/CVPR25-MINE) | 
| 32883 | [NLPrompt: Noise-Label Prompt Learning for Vision-Language Models](2412.01256) | Bikang Pan | Highlight | 0 | [Code😻](https://github.com/qunovo/NLPrompt) | 
| 32905 | [STiL: Semi-supervised Tabular-Image Learning for Comprehensive Task-Relevant Information Exploration in Multimodal Classification](2503.06277) | Siyi Du | Poster | 0 | [Code😻](https://github.com/siyi-wind/STiL) | 
| 32918 | [DFormerv2: Geometry Self-Attention for RGBD Semantic Segmentation](2504.04701) | Bo-Wen Yin | Poster | 1 | [Code😻](https://github.com/VCIP-RGBD/DFormer) | 
| 32930 | [Enhancing Testing-Time Robustness for Trusted Multi-View Classification in the Wild](https://openaccess.thecvf.com/content/CVPR2025/papers/Liu_Enhancing_Testing-Time_Robustness_for_Trusted_Multi-View_Classification_in_the_Wild_CVPR_2025_paper.pdf) | Wei Liu | Poster | 0 | 😾 | 
| 32961 | [Dual-view X-ray Detection: Can AI Detect Prohibited Items from Dual-view X-ray Images like Humans?](https://openaccess.thecvf.com/content/CVPR2025/papers/Tao_Dual-view_X-ray_Detection_Can_AI_Detect_Prohibited_Items_from_Dual-view_CVPR_2025_paper.pdf) | Renshuai Tao | Poster | 2 | [Code😻](https://github.com/rstao-bjtu/LDXray/) | 
| 33075 | [Crab: A Unified Audio-Visual Scene Understanding Model with Explicit Cooperation](2503.13068) | Henghui Du | Poster | 0 | [Code😻](https://github.com/GeWu-Lab/Crab) | 
| 33076 | [Keep the Balance: A Parameter-Efficient Symmetrical Framework for RGB+X Semantic Segmentation](https://openaccess.thecvf.com/content/CVPR2025/papers/Cai_Keep_the_Balance_A_Parameter-Efficient_Symmetrical_Framework_for_RGBX_Semantic_CVPR_2025_paper.pdf) | Jiaxin Cai | Poster | 0 | 😾 | 
| 33163 | [Align-KD: Distilling Cross-Modal Alignment Knowledge for Mobile Vision-Language Large Model Enhancement](https://openaccess.thecvf.com/content/CVPR2025/papers/Feng_Align-KD_Distilling_Cross-Modal_Alignment_Knowledge_for_Mobile_Vision-Language_Large_Model_CVPR_2025_paper.pdf) | Qianhan Feng | Poster | 0 | 😾 | 
| 33226 | [Enhanced Visual-Semantic Interaction with Tailored Prompts for Pedestrian Attribute Recognition](https://openaccess.thecvf.com/content/CVPR2025/papers/Wu_Enhanced_Visual-Semantic_Interaction_with_Tailored_Prompts_for_Pedestrian_Attribute_Recognition_CVPR_2025_paper.pdf) | Junyi Wu | Highlight | 0 | 😾 | 
| 33366 | [Provoking Multi-modal Few-Shot LVLM via Exploration-Exploitation In-Context Learning](https://openaccess.thecvf.com/content/CVPR2025/papers/Chen_Provoking_Multi-modal_Few-Shot_LVLM_via_Exploration-Exploitation_In-Context_Learning_CVPR_2025_paper.pdf) | Cheng Chen | Poster | 0 | 😾 | 
| 33484 | [FOCUS: Knowledge-enhanced Adaptive Visual Compression for Few-shot Whole Slide Image Classification](2411.14743) | Zhengrui Guo | Poster | 3 | [Code😻](https://github.com/dddavid4real/FOCUS) | 
| 33522 | [Fast and Accurate Gigapixel Pathological Image Classification with Hierarchical Distillation Multi-Instance Learning](2502.21130) | Jiuyang Dong | Poster | 0 | [Code😻](https://github.com/JiuyangDong/HDMIL) | 
| 33528 | [BioX-CPath: Biologically-driven Explainable Diagnostics for Multistain IHC Computational Pathology](https://openaccess.thecvf.com/content/CVPR2025/papers/Gallagher-Syed_BioX-CPath_Biologically-driven_Explainable_Diagnostics_for_Multistain_IHC_Computational_Pathology_CVPR_2025_paper.pdf) | Amaya Gallagher-Syed | Poster | 0 | [Code😻](https://github.com/AmayaGS/BioX-CPath) | 
| 33634 | [UWAV: Uncertainty-weighted Weakly-supervised Audio-Visual Video Parsing](2505.09615) | Yung-Hsuan Lai | Poster | 0 | [Code😻](https://github.com/Franklin905/UWAV) | 
| 33663 | [Visual and Semantic Prompt Collaboration for Generalized Zero-Shot Learning](2503.23030) | Huajie Jiang | Poster | 0 | 😾 | 
| 33669 | [Towards General Visual-Linguistic Face Forgery Detection](https://openaccess.thecvf.com/content/CVPR2025/papers/Sun_Towards_General_Visual-Linguistic_Face_Forgery_Detection_CVPR_2025_paper.pdf) | Ke Sun | Poster | 18 | [Code😻](https://github.com/skJack/VLFFD.git) | 
| 33688 | [Multi-modal Medical Diagnosis via Large-small Model Collaboration](https://openaccess.thecvf.com/content/CVPR2025/papers/Chen_Multi-modal_Medical_Diagnosis_via_Large-small_Model_Collaboration_CVPR_2025_paper.pdf) | Wanyi Chen | Poster | 0 | [Code😻](https://github.com/Zoew420/AdaCoMed) | 
| 33750 | [Multi-Resolution Pathology-Language Pre-training Model with Text-Guided Visual Representation](2504.18856) | Shahad Albastaki | Poster | 0 | [Code😻](https://github.com/BasitAlawode/MR-PLIP) | 
| 33751 | [Discovering Fine-Grained Visual-Concept Relations by Disentangled Optimal Transport Concept Bottleneck Models](2505.07209) | Yan Xie | Poster | 1 | 😾 | 
| 34120 | [MExD: An Expert-Infused Diffusion Model for Whole-Slide Image Classification](2503.12401) | Jianwei Zhao | Poster | 0 | 😾 | 
| 34176 | [Generative Modeling of Class Probability for Multi-Modal Representation Learning](2503.17417) | JungKyoo Shin | Highlight | 1 | 😾 | 
| 34205 | [AVF-MAE++: Scaling Affective Video Facial Masked Autoencoders via Efficient Audio-Visual Self-Supervised Learning](https://cvpr.thecvf.com/virtual/2025/poster/34205) | Xuecheng Wu | Poster | 0 | 😾 | 
| 34390 | [Enhancing Vision-Language Compositional Understanding with Multimodal Synthetic Data](2503.01167) | Haoxin Li | Poster | 1 | 😾 | 
| 34433 | [Learning Heterogeneous Tissues with Mixture of Experts for Gigapixel Whole Slide Images](https://openaccess.thecvf.com/content/CVPR2025/papers/Wu_Learning_Heterogeneous_Tissues_with_Mixture_of_Experts_for_Gigapixel_Whole_CVPR_2025_paper.pdf) | Junxian Wu | Poster | 0 | [Code😻](https://github.com/wjx-error/PAMoE) | 
| 34647 | [STING-BEE: Towards Vision-Language Model for Real-World X-ray Baggage Security Inspection](https://openaccess.thecvf.com/content/CVPR2025/papers/Velayudhan_STING-BEE_Towards_Vision-Language_Model_for_Real-World_X-ray_Baggage_Security_Inspection_CVPR_2025_paper.pdf) | Divya Velayudhan | Highlight | 0 | 😾 | 
| 34653 | [MMTL-UniAD: A Unified Framework for Multimodal and Multi-Task Learning in Assistive Driving Perception](https://openaccess.thecvf.com/content/CVPR2025/papers/Liu_MMTL-UniAD_A_Unified_Framework_for_Multimodal_and_Multi-Task_Learning_in_CVPR_2025_paper.pdf) | Wenzhuo Liu | Poster | 1 | [Code😻](https://github.com/Wenzhuo-Liu/MMTL-UniAD) | 
| 34686 | [Recover and Match: Open-Vocabulary Multi-Label Recognition through Knowledge-Constrained Optimal Transport](2503.15337) | Hao Tan | Poster | 0 | [Code😻](https://github.com/EricTan7/RAM) | 
| 34763 | [Language-Guided Audio-Visual Learning for Long-Term Sports Assessment](https://openaccess.thecvf.com/content/CVPR2025/papers/Xu_Language-Guided_Audio-Visual_Learning_for_Long-Term_Sports_Assessment_CVPR_2025_paper.pdf) | Huangbiao Xu | Poster | 0 | [Code😻](https://github.com/XuHuangbiao/MLA VL) | 
| 34969 | [Semantic-guided Cross-Modal Prompt Learning for Skeleton-based Zero-shot Action Recognition](https://openaccess.thecvf.com/content/CVPR2025/papers/Zhu_Semantic-guided_Cross-Modal_Prompt_Learning_for_Skeleton-based_Zero-shot_Action_Recognition_CVPR_2025_paper.pdf) | Anqi Zhu | Poster | 0 | 😾 | 
| 35071 | [CPath-Omni: A Unified Multimodal Foundation Model for Patch and Whole Slide Image Analysis in Computational Pathology](https://openaccess.thecvf.com/content/CVPR2025/papers/Sun_CPath-Omni_A_Unified_Multimodal_Foundation_Model_for_Patch_and_Whole_CVPR_2025_paper.pdf) | Yuxuan Sun | Poster | 4 | 😾 | 
| 35072 | [2DMamba: Efficient State Space Model for Image Representation with Applications on Giga-Pixel Whole Slide Image Classification](2412.00678) | Jingwei Zhang | Poster | 5 | [Code😻](https://github.com/AtlasAnalyticsLab/2DMamba) | 
| 35074 | [SPARC: Score Prompting and Adaptive Fusion for Zero-Shot Multi-Label Recognition in Vision-Language Models](2502.16911) | Kevin Miller | Poster | 0 | [Code😻](https://github.com/kjmillerCURIS/SPARC) | 
| 35216 | [Incomplete Multi-View Multi-label Learning via Disentangled Representation and Label Semantic Embedding](https://openaccess.thecvf.com/content/CVPR2025/papers/Yan_Incomplete_Multi-View_Multi-label_Learning_via_Disentangled_Representation_and_Label_Semantic_CVPR_2025_paper.pdf) | Xu Yan | Poster | 0 | 😾 | 
</details>

<details>
  <summary><strong>AAAI 2025</strong>（共筛选出 87 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| article-31977 | [External Reliable Information-enhanced Multimodal Contrastive Learning for Fake News Detection](https://ojs.aaai.org/index.php/AAAI/article/view/31977/34132) | Biwei Cao | Technical | 0 | [Code😻](https://github.com/Tarasom123/ERIC-FND) | 
| article-31980 | [mmFAS: Multimodal Face Anti-Spoofing Using Multi-Level Alignment and Switch-Attention Fusion](https://ojs.aaai.org/index.php/AAAI/article/view/31980/34135) | Geng Chen | Technical | 0 | 😾 | 
| article-32001 | [CoPRA: Bridging Cross-domain Pretrained Sequence Models with Complex Structures for Protein-RNA Binding Affinity Prediction](https://ojs.aaai.org/index.php/AAAI/article/view/32001/34156) | Rong Han | Technical | 2 | [Code😻](https://github.com/hanrthu/CoPRA) | 
| article-32010 | [PRAGA: Prototype-aware Graph Adaptive Aggregation for Spatial Multi-modal Omics Analysis](https://ojs.aaai.org/index.php/AAAI/article/view/32010/34165) | Xinlei Huang | Technical | 3 | [Code😻](https://github.com/Xubin-s-Lab/PRAGA) | 
| article-32036 | [DAMMFND: Domain-Aware Multimodal Multi-view Fake News Detection](https://ojs.aaai.org/index.php/AAAI/article/view/32036/34191) | Weihai Lu | Technical | 0 | [Code😻](https://github.com/luweihai/DAMMFND) | 
| article-32042 | [Multi-modal Deepfake Detection via Multi-task Audio-Visual Prompt Learning](https://ojs.aaai.org/index.php/AAAI/article/view/32042/34197) | Hui Miao | Technical | 0 | 😾 | 
| article-32114 | [Dynamic Interactive Bimodal Hypergraph Networks for Emotion Recognition in Conversations](https://ojs.aaai.org/index.php/AAAI/article/view/32114/34269) | Xuping Chen | Technical | 0 | 😾 | 
| article-32131 | [Semi-IIN: Semi-Supervised Intra-Inter Modal Interaction Learning Network for Multimodal Sentiment Analysis](https://ojs.aaai.org/index.php/AAAI/article/view/32131/34286) | Jinhao Lin | Technical | 0 | [Code😻](https://github.com/flow-ljh/Semi-IIN) | 
| article-32134 | [Multi-to-Single: Reducing Multimodal Dependency in Emotion Recognition Through Contrastive Learning](https://ojs.aaai.org/index.php/AAAI/article/view/32134/34289) | Yan-Kai Liu | Technical | 0 | [Code😻](https://github.com/Arcee-LYK/Multi-to-Single) | 
| article-32145 | [Pose as a Modality: A Psychology-Inspired Network for Personality Recognition with a New Multimodal Dataset](https://ojs.aaai.org/index.php/AAAI/article/view/32145/34300) | Bin Tang | Technical | 0 | 😾 | 
| article-32149 | [BIG-FUSION: Brain-Inspired Global-Local Context Fusion Framework for Multimodal Emotion Recognition in Conversations](https://ojs.aaai.org/index.php/AAAI/article/view/32149/34304) | Yusong Wang | Technical | 0 | 😾 | 
| article-32170 | [CA-MLIF: Cross-Attention and Multimodal Low-Rank Interaction Fusion Framework for Tumor Prognostic Prediction](https://ojs.aaai.org/index.php/AAAI/article/view/32170/34325) | Yajun An | Technical | 0 | 😾 | 
| article-32173 | [ProtoArgNet: Interpretable Image Classification with Super-Prototypes and Argumentation](https://ojs.aaai.org/index.php/AAAI/article/view/32173/34328) | Hamed Ayoobi | Technical | 0 | 😾 | 
| article-32204 | [Adversarial Learning Under Hybrid Perturbations for Robust Acute Lymphoblastic Leukemia Classification](https://ojs.aaai.org/index.php/AAAI/article/view/32204/34359) | Jie Chen | Technical | 0 | 😾 | 
| article-32321 | [ParseCaps: An Interpretable Parsing Capsule Network for Medical Image Diagnosis](https://ojs.aaai.org/index.php/AAAI/article/view/32321/34476) | Xinyu Geng | Technical | 1 | 😾 | 
| article-32325 | [Queryable Prototype Multiple Instance Learning with Vision-Language Models for Incremental Whole Slide Image Classification](https://ojs.aaai.org/index.php/AAAI/article/view/32325/34480) | Jiaxiang Gou | Technical | 2 | [Code😻](https://github.com/can-can-ya/QPMIL-VL) | 
| article-32332 | [Surgical Workflow Recognition and Blocking Effectiveness Detection in Laparoscopic Liver Resection with Pringle Maneuver](https://ojs.aaai.org/index.php/AAAI/article/view/32332/34487) | Diandian Guo | Technical | 2 | [Code😻](https://github.com/RascalGdd/PmNet) | 
| article-32342 | [LLaVA Needs More Knowledge: Retrieval Augmented Natural Language Generation with Knowledge Graph for Explaining Thoracic Pathologies](https://ojs.aaai.org/index.php/AAAI/article/view/32342/34497) | Ameer Hamza | Technical | 1 | [Code😻](https://github.com/ailab-kyunghee/KG-LLaV A) | 
| article-32432 | [Exploring More from Multiple Gait Modalities for Human Identification](https://ojs.aaai.org/index.php/AAAI/article/view/32432/34587) | Dongyang Jin | Technical | 0 | [Code😻](https://github.com/ShiqiYu/OpenGait) | 
| article-32489 | [FEAST-Mamba: FEAture and SpaTial Aware Mamba Network with Bidirectional Orthogonal Fusion for Cross-Modal Point Cloud Segmentation](https://ojs.aaai.org/index.php/AAAI/article/view/32489/34644) | Chade Li | Technical | 0 | 😾 | 
| article-32501 | [M²RL-Net: Multi-View and Multi-Level Relation Learning Network for Weakly-Supervised Image Forgery Detection](https://ojs.aaai.org/index.php/AAAI/article/view/32501/34656) | Jiafeng Li | Technical | 0 | 😾 | 
| article-32507 | [FD2-Net: Frequency-Driven Feature Decomposition Network for Infrared-Visible Object Detection](https://ojs.aaai.org/index.php/AAAI/article/view/32507/34662) | Ke Li | Technical | 2 | 😾 | 
| article-32534 | [Text and Image Are Mutually Beneficial: Enhancing Training-Free Few-Shot Classification with CLIP](https://ojs.aaai.org/index.php/AAAI/article/view/32534/34689) | Yayuan Li | Technical | 0 | [Code😻](https://github.com/lyymuwu/TIMO) | 
| article-32545 | [Semantic-guided Masked Mutual Learning for Multi-modal Brain Tumor Segmentation with Arbitrary Missing Modalities](https://ojs.aaai.org/index.php/AAAI/article/view/32545/34700) | Guoyan Liang | Technical | 0 | 😾 | 
| article-32558 | [Leveraging RGB-D Data with Cross-Modal Context Mining for Glass Surface Detection](https://ojs.aaai.org/index.php/AAAI/article/view/32558/34713) | Jiaying Lin | Technical | 1 | 😾 | 
| article-32559 | [Standing on the Shoulders of Giants: Reprogramming Visual-Language Model for General Deepfake Detection](https://ojs.aaai.org/index.php/AAAI/article/view/32559/34714) | Kaiqing Lin | Technical | 2 | 😾 | 
| article-32570 | [Incomplete Modality Disentangled Representation for Ophthalmic Disease Grading and Diagnosis](https://ojs.aaai.org/index.php/AAAI/article/view/32570/34725) | Chengzhi Liu | Technical | 1 | 😾 | 
| article-32607 | [CLIP-PCQA: Exploring Subjective-Aligned Vision-Language Modeling for Point Cloud Quality Assessment](https://ojs.aaai.org/index.php/AAAI/article/view/32607/34762) | Yating Liu | Technical | 0 | [Code😻](https://github.com/Olivialyt/CLIP-PCQA) | 
| article-32642 | [A Trusted Lesion-assessment Network for Interpretable Diagnosis of Coronary Artery Disease in Coronary CT Angiography](https://ojs.aaai.org/index.php/AAAI/article/view/32642/34797) | Xinghua Ma | Technical | 0 | [Code😻](https://github.com/PerceptionComputingLab/CAD Diagnosis) | 
| article-32684 | [Beyond Text: Fine-Grained Multi-Modal Fact Verification with Hypergraph Transformers](https://ojs.aaai.org/index.php/AAAI/article/view/32684/34839) | Hui Pang | Technical | 0 | 😾 | 
| article-32744 | [SKI Models: Skeleton Induced Vision-Language Embeddings for Understanding Activities of Daily Living](https://ojs.aaai.org/index.php/AAAI/article/view/32744/34899) | Arkaprava Sinha | Technical | 1 | [Code😻](https://github.com/thearkaprava/SKI-Models) | 
| article-32745 | [JoVALE: Detecting Human Actions in Video Using Audiovisual and Language Contexts](https://ojs.aaai.org/index.php/AAAI/article/view/32745/34900) | Taein Son | Technical | 0 | [Code😻](https://github.com/taeiin/AAAI2025-JoVALE) | 
| article-32772 | [C2P-CLIP: Injecting Category Common Prompt in CLIP to Enhance Generalization in Deepfake Detection](https://ojs.aaai.org/index.php/AAAI/article/view/32772/34927) | Chuangchuang Tan | Technical | 10 | 😾 | 
| article-32807 | [Overcoming Heterogeneous Data in Federated Medical Vision-Language Pre-training: A Triple-Embedding Model Selector Approach](https://ojs.aaai.org/index.php/AAAI/article/view/32807/34962) | Aowen Wang | Technical | 0 | [Code😻](https://github.com/NBT-AILAB/PMS-FM) | 
| article-32841 | [VLScene: Vision-Language Guidance Distillation for Camera-Based 3D Semantic Scene Completion](https://ojs.aaai.org/index.php/AAAI/article/view/32841/34996) | Meng Wang | Technical | 0 | [Code😻](https://github.com/willemeng/VLScene) | 
| article-32845 | [Meme Trojan: Backdoor Attacks Against Hateful Meme Detection via Cross-Modal Triggers](https://ojs.aaai.org/index.php/AAAI/article/view/32845/35000) | Ruofei Wang | Technical | 0 | 😾 | 
| article-32856 | [CNC: Cross-modal Normality Constraint for Unsupervised Multi-class Anomaly Detection](https://ojs.aaai.org/index.php/AAAI/article/view/32856/35011) | Xiaolei Wang | Technical | 1 | [Code😻](https://github.com/cvddl/CNC) | 
| article-32869 | [Enhancing Fine-Grained Vision-Language Pretraining with Negative Augmented Samples](https://ojs.aaai.org/index.php/AAAI/article/view/32869/35024) | Yeyuan Wang | Technical | 2 | 😾 | 
| article-32908 | [ICM-Assistant: Instruction-tuning Multimodal Large Language Models for Rule-based Explainable Image Content Moderation](https://ojs.aaai.org/index.php/AAAI/article/view/32908/35063) | Mengyang Wu | Technical | 0 | [Code😻](https://github.com/zhaoyuzhi/ICM-Assistant) | 
| article-32970 | [PDDM: Pseudo Depth Diffusion Model for RGB-PD Semantic Segmentation Based in Complex Indoor Scenes](https://ojs.aaai.org/index.php/AAAI/article/view/32970/35125) | Xinhua Xu | Technical | 0 | [Code😻](https://github.com/Oleki-xxh/PDDM) | 
| article-33050 | [Building a Multi-modal Spatiotemporal Expert for Zero-shot Action Recognition with CLIP](https://ojs.aaai.org/index.php/AAAI/article/view/33050/35205) | Yating Yu | Technical | 0 | [Code😻](https://github.com/Mia-YatingYu/STDD) | 
| article-33064 | [GaitCycFormer: Leveraging Gait Cycles and Transformers for Gait Emotion Recognition.](https://ojs.aaai.org/index.php/AAAI/article/view/33064/35219) | Qingyang Zeng | Technical | 0 | 😾 | 
| article-33090 | [SIGraph: Saliency Image-Graph Network for Retinal Disease Classification in Fundus Image](https://ojs.aaai.org/index.php/AAAI/article/view/33090/35245) | Peng Zhang | Technical | 0 | 😾 | 
| article-33118 | [Cross-Modal Few-Shot Learning with Second-Order Neural Ordinary Differential Equations](https://ojs.aaai.org/index.php/AAAI/article/view/33118/35273) | Yi Zhang | Technical | 1 | 😾 | 
| article-33134 | [Multimodal Class-aware Semantic Enhancement Network for Audio-Visual Video Parsing](https://ojs.aaai.org/index.php/AAAI/article/view/33134/35289) | Pengcheng Zhao | Technical | 3 | 😾 | 
| article-33154 | [OODML: Whole Slide Image Classification Meets Online Pseudo-Supervision and Dynamic Mutual Learning](https://ojs.aaai.org/index.php/AAAI/article/view/33154/35309) | Tingting Zheng | Technical | 0 | 😾 | 
| article-33157 | [MMPF: Multi-Modal Perception Framework for Abnormal Medical Condition Detection](https://ojs.aaai.org/index.php/AAAI/article/view/33157/35312) | Chuyi Zhong | Technical | 0 | 😾 | 
| article-33242 | [Revisiting Multimodal Emotion Recognition in Conversation from the Perspective of Graph Spectrum](https://ojs.aaai.org/index.php/AAAI/article/view/33242/35397) | Wei Ai | Technical | 24 | 😾 | 
| article-33246 | [Cross-modal Multi-task Learning for Multimedia Event Extraction](https://ojs.aaai.org/index.php/AAAI/article/view/33246/35401) | Jianwei Cao | Technical | 0 | 😾 | 
| article-33388 | [MDFG: Multi-Dimensional Fine-Grained Modeling for Fatigue Detection](https://ojs.aaai.org/index.php/AAAI/article/view/33388/35543) | Mei Wang | Technical | 0 | 😾 | 
| article-33582 | [Decomposing and Fusing Intra- and Inter-Sensor Spatio-Temporal Signal for Multi-Sensor Wearable Human Activity Recognition](https://ojs.aaai.org/index.php/AAAI/article/view/33582/35737) | Haoyu Xie | Technical | 0 | 😾 | 
| article-33675 | [Counterfactual Debiasing for Physical Audiovisual Commonsense Reasoning](https://ojs.aaai.org/index.php/AAAI/article/view/33675/35830) | Daoming Zong | Technical | 0 | 😾 | 
| article-33736 | [Semi-Supervised Multimodal Classification Through Learning from Modal and Strategic Complementarities](https://ojs.aaai.org/index.php/AAAI/article/view/33736/35891) | Junchi Chen | Technical | 0 | 😾 | 
| article-33762 | [ZeroHAR: Sensor Context Augments Zero-Shot Wearable Action Recognition](https://ojs.aaai.org/index.php/AAAI/article/view/33762/35917) | Ranak Roy Chowdhury | Technical | 0 | 😾 | 
| article-33770 | [Few-Shot Audio-Visual Class-Incremental Learning with Temporal Prompting and Regularization](https://ojs.aaai.org/index.php/AAAI/article/view/33770/35925) | Yawen Cui | Technical | 0 | 😾 | 
| article-33849 | [An Optimal Transport-based Latent Mixer for Robust Multi-modal Learning](https://ojs.aaai.org/index.php/AAAI/article/view/33849/36004) | Fengjiiao Gong | Technical | 0 | 😾 | 
| article-34028 | [Semi-Supervised Multi-View Multi-Label Learning with View-Specific Transformer and Enhanced Pseudo-Label](https://ojs.aaai.org/index.php/AAAI/article/view/34028/36183) | Quanjiang Li | Technical | 0 | 😾 | 
| article-34080 | [Vision-aware Multimodal Prompt Tuning for Uploadable Multi-source Few-shot Domain Adaptation](https://ojs.aaai.org/index.php/AAAI/article/view/34080/36235) | Kuanghong Liu | Technical | 0 | [Code😻](https://github.com/lkh-meredith/VAMP-UMFDA) | 
| article-34116 | [Like an Ophthalmologist: Dynamic Selection Driven Multi-View Learning for Diabetic Retinopathy Grading](https://ojs.aaai.org/index.php/AAAI/article/view/34116/36271) | Xiaoling Luo | Technical | 0 | [Code😻](https://github.com/xqh180110910537/SMVDR) | 
| article-34141 | [Causal Inference over Visual-Semantic-Aligned Graph for Image Classification](https://ojs.aaai.org/index.php/AAAI/article/view/34141/36296) | Lei Meng | Technical | 0 | 😾 | 
| article-34176 | [Incomplete Multi-View Multi-Label Classification via Diffusion-Guided Redundancy Removal](https://ojs.aaai.org/index.php/AAAI/article/view/34176/36331) | Shilong Ou | Technical | 0 | 😾 | 
| article-34389 | [Blend the Separated: Mixture of Synergistic Experts for Data-Scarcity Drug-Target Interaction Prediction](https://ojs.aaai.org/index.php/AAAI/article/view/34389/36544) | Xinlong Zhai | Technical | 0 | [Code😻](https://github.com/BUPT-GAMMA/MoseDTI) | 
| article-34391 | [Text-Guided Nonverbal Enhancement Based on Modality-Invariant and -Specific Representations for Video Speaking Style Recognition](https://ojs.aaai.org/index.php/AAAI/article/view/34391/36546) | Beibei Zhang | Technical | 0 | 😾 | 
| article-34401 | [DiffCLIP: Few-shot Language-driven Multimodal Classifier](https://ojs.aaai.org/index.php/AAAI/article/view/34401/36556) | Jiaqing Zhang | Technical | 1 | [Code😻](https://github.com/icey-zhang/DiffCLIP) | 
| article-34572 | [DEQA: Descriptions Enhanced Question-Answering Framework for Multimodal Aspect-Based Sentiment Analysis](https://ojs.aaai.org/index.php/AAAI/article/view/34572/36727) | Zhixin Han | Technical | 0 | 😾 | 
| article-34671 | [DECT: Harnessing LLM-assisted Fine-Grained Linguistic Knowledge and Label-Switched and Label-Preserved Data Generation for Diagnosis of Alzheimer’s Disease](https://ojs.aaai.org/index.php/AAAI/article/view/34671/36826) | Tingyu Mo | Technical | 0 | 😾 | 
| article-34687 | [Mental-Perceiver: Audio-Textual Multi-Modal Learning for Estimating Mental Disorders](https://ojs.aaai.org/index.php/AAAI/article/view/34687/36842) | Jinghui Qin | Technical | 0 | [Code😻](https://github.com/shuyeit/mmpsy-data) | 
| article-34711 | [Multi-Grained Query-Guided Set Prediction Network for Grounded Multimodal Named Entity Recognition](https://ojs.aaai.org/index.php/AAAI/article/view/34711/36866) | Jielong Tang | Technical | 1 | [Code😻](https://github.com/tangjielong928/mqspn) | 
| article-34741 | [Enhancing Audiovisual Speech Recognition Through Bifocal Preference Optimization](https://ojs.aaai.org/index.php/AAAI/article/view/34741/36896) | Yihan Wu | Technical | 0 | [Code😻](https://github.com/espnet/espnet) | 
| article-34755 | [MSE-Adapter: A Lightweight Plugin Endowing LLMs with the Capability to Perform Multimodal Sentiment Analysis and Emotion Recognition](https://ojs.aaai.org/index.php/AAAI/article/view/34755/36910) | Yang Yang | Technical | 0 | [Code😻](https://github.com/AZYoung233/MSE-Adapter) | 
| article-34997 | [Dust-Mamba: An Efficient Dust Storm Detection Network with Multiple Data Sources](https://ojs.aaai.org/index.php/AAAI/article/view/34997/37152) | Cong Bai | Technical | 0 | 😾 | 
| article-35002 | [FoMo: Multi-Modal, Multi-Scale and Multi-Task Remote Sensing Foundation Models for Forest Monitoring](https://ojs.aaai.org/index.php/AAAI/article/view/35002/37157) | Nikolaos Ioannis Bountos | Technical | 0 | [Code😻](https://github.com/RolnickLab/FoMo-Bench) | 
| article-35023 | [Enhancing Vision-Language Models with Morphological and Taxonomic Knowledge: Towards Coral Recognition for Ocean Health](https://ojs.aaai.org/index.php/AAAI/article/view/35023/37178) | Hongyong Han | Technical | 0 | 😾 | 
| article-35024 | [UrbanVLP: Multi-Granularity Vision-Language Pretraining for Urban Socioeconomic Indicator Prediction](https://ojs.aaai.org/index.php/AAAI/article/view/35024/37179) | Xixuan Hao | Technical | 1 | [Code😻](https://github.com/skyerhxx/UrbanVLP) | 
| article-35031 | [Accessible, At-Home Detection of Parkinson’s Disease via Multi-Task Video Analysis](https://ojs.aaai.org/index.php/AAAI/article/view/35031/37186) | Md Saiful Islam | Technical | 1 | [Code😻](https://github.com/ROC-HCI/UFNet) | 
| article-35033 | [VidSole: A Multimodal Dataset for Joint Kinetics Quantification and Disease Detection with Deep Learning](https://ojs.aaai.org/index.php/AAAI/article/view/35033/37188) | Archit Kambhamettu | Technical | 0 | 😾 | 
| article-35037 | [KD-MSLRT: Lightweight Sign Language Recognition Model Based on Mediapipe and 3D to 1D Knowledge Distillation](https://ojs.aaai.org/index.php/AAAI/article/view/35037/37192) | Yulong Li | Technical | 0 | 😾 | 
| article-35041 | [NaFV-Net: An Adversarial Four-view Network for Mammogram Classification](https://ojs.aaai.org/index.php/AAAI/article/view/35041/37196) | Feng Lu | Technical | 0 | 😾 | 
| article-35047 | [MCAT: Visual Query-Based Localization of Standard Anatomical Clips in Fetal Ultrasound Videos Using Multi-Tier Class-Aware Token Transformer](https://ojs.aaai.org/index.php/AAAI/article/view/35047/37202) | Divyanshu Mishra | Technical | 0 | 😾 | 
| article-35048 | [Pioneering Explainable Video Fact-Checking with a New Dataset and Multi-role Multimodal Model Approach](https://ojs.aaai.org/index.php/AAAI/article/view/35048/37203) | Kaipeng Niu | Technical | 0 | [Code😻](https://github.com/MeiTaylor/TRUE-3MFact) | 
| article-35054 | [Incongruent Multimodal Federated Learning for Medical Vision and Language-based Multi-label Disease Detection](https://ojs.aaai.org/index.php/AAAI/article/view/35054/37209) | Pramit Saha | Technical | 0 | 😾 | 
| article-35254 | [Multi-Modal Hand-to-Mouth Gesture Recognition in Activity-Oriented RGB-Thermal Footage (Student Abstract)](https://ojs.aaai.org/index.php/AAAI/article/view/35254/37409) | Glenn Fernandes | Technical | 0 | 😾 | 
| article-35283 | [3M-Game: Multi-Modal Multi-Task Multi-Teacher Learning for Game Event Detection (Student Abstract)](https://ojs.aaai.org/index.php/AAAI/article/view/35283/37438) | Thye Shan Ng | Technical | 0 | [Code😻](https://github.com/adlnlp/3m game) | 
| article-35322 | [Understanding Unique Behavioral Patterns through Multimodal Analysis of Eye-Hand Coordination in Autistic Children (Student Abstract)](https://ojs.aaai.org/index.php/AAAI/article/view/35322/37477) | Emily Yu | Technical | 0 | [Code😻](https://github.com/mathjams/AAAI 2024) | 
| article-35327 | [Utilizing Vision-Language Models for Detection of Leaf-Based Diseases in Tomatoes](https://ojs.aaai.org/index.php/AAAI/article/view/35327/37482) | James Blossom Eleojo | Technical | 0 | 😾 | 
| article-35417 | [Pre-Trained Vision-Language Models as Noisy Partial Annotators](https://ojs.aaai.org/index.php/AAAI/article/view/35417/37572) | Qian-Wei Wang | Technical | 0 | [Code😻](https://github.com/Portugas0807/Co-Reg) | 
| article-35428 | [Global-Semantic Alignment Distillation for Partial Multi-view Classification](https://ojs.aaai.org/index.php/AAAI/article/view/35428/37583) | Xiaoli Wang | Technical | 0 | 😾 | 
</details>

<details>
  <summary><strong>IJCAI 2024</strong>（共筛选出 18 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| paper1029 | [XGA-Osteo: Towards XAI-Enabled Knee Osteoarthritis Diagnosis with Adversarial Learning](https://www.ijcai.org/proceedings/2024/1029.pdf) | Hieu Phan | Poster | 0 | 😾 | 
| paper200 | [Class-consistent Contrastive Learning Driven Cross-dimensional Transformer for 3D Medical Image Classification](https://www.ijcai.org/proceedings/2024/0200.pdf) | Qikui Zhu | Poster | 2 | 😾 | 
| paper203 | [CLIP-FSAC: Boosting CLIP for Few-Shot Anomaly Classification with Synthetic Anomalies](https://www.ijcai.org/proceedings/2024/0203.pdf) | Zuo Zuo | Poster | 6 | 😾 | 
| paper340 | [Multi-level Disentangling Network for Cross-Subject Emotion Recognition Based on Multimodal Physiological Signals](https://www.ijcai.org/proceedings/2024/0340.pdf) | Ziyu Jia | Poster | 5 | 😾 | 
| paper341 | [VSGT: Variational Spatial and Gaussian Temporal Graph Models for EEG-based Emotion Recognition](https://www.ijcai.org/proceedings/2024/0341.pdf) | Chenyu Liu | Poster | 1 | 😾 | 
| paper345 | [DBPNet: Dual-Branch Parallel Network with Temporal-Frequency Fusion for Auditory Attention Detection](https://www.ijcai.org/proceedings/2024/0345.pdf) | Qinke Ni | Poster | 15 | 😾 | 
| paper440 | [Core-Structures-Guided Multi-Modal Classification Neural Architecture Search](https://www.ijcai.org/proceedings/2024/0440.pdf) | Pinhan Fu | Poster | 3 | [Code😻](https://github.com/fupinhan123/CSG-NAS) | 
| paper578 | [TAI++: Text as Image for Multi-Label Image Classification by Co-Learning Transferable Prompt](https://www.ijcai.org/proceedings/2024/0578.pdf) | Xiangyu Wu | Poster | 3 | [Code😻](https://github.com/njustkmg/PVP) | 
| paper621 | [Cross-View Contrastive Fusion for Enhanced Molecular Property Prediction](https://www.ijcai.org/proceedings/2024/0621.pdf) | Yan Zheng | Poster | 1 | 😾 | 
| paper636 | [Balancing Multimodal Learning via Online Logit Modulation](https://www.ijcai.org/proceedings/2024/0636.pdf) | Daoming Zong | Poster | 0 | 😾 | 
| paper650 | [ATTA: Adaptive Test-Time Adaptation for Multi-Modal Sleep Stage Classification](https://www.ijcai.org/proceedings/2024/0650.pdf) | Ziyu Jia | Poster | 2 | 😾 | 
| paper655 | [OUCopula: Bi-Channel Multi-Label Copula-Enhanced Adapter-Based CNN for Myopia Screening Based on OU-UWF Images](https://www.ijcai.org/proceedings/2024/0655.pdf) | Yang Li | Poster | 3 | [Code😻](github.com/Charley-HUANG/OUCopula) | 
| paper658 | [SCTrans: Multi-scale scRNA-seq Sub-vector Completion Transformer for Gene-selective Cell Type Annotation](https://www.ijcai.org/proceedings/2024/0658.pdf) | Lu Lin | Poster | 0 | 😾 | 
| paper671 | [ZeroDDI: A Zero-Shot Drug-Drug Interaction Event Prediction Method with Semantic Enhanced Learning and Dual-modal Uniform Alignment](https://www.ijcai.org/proceedings/2024/0671.pdf) | Ziyan Wang | Poster | 2 | [Code😻](https://github.com/wzy-Sarah/ZeroDDI) | 
| paper681 | [Heterogeneous Causal Metapath Graph Neural Network for Gene-Microbe-Disease Association Prediction](https://www.ijcai.org/proceedings/2024/0681.pdf) | Kexin Zhang | Poster | 1 | 😾 | 
| paper739 | [TFCD: Towards Multi-modal Sarcasm Detection via Training-Free Counterfactual Debiasing](https://www.ijcai.org/proceedings/2024/0739.pdf) | Zhihong Zhu | Poster | 10 | 😾 | 
| paper795 | [Functional Graph Convolutional Networks: A Unified Multi-task and Multi-modal Learning Framework to Facilitate Health and Social-Care Insights](https://www.ijcai.org/proceedings/2024/0795.pdf) | Tobia Boschi | Poster | 1 | [Code😻](https://github.com/IBM/funGCN) | 
| paper799 | [FairReFuse: Referee-Guided Fusion for Multi-Modal Causal Fairness in Depression Detection](https://www.ijcai.org/proceedings/2024/0799.pdf) | Jiaee Cheong | Poster | 6 | 😾 | 
</details>

<details>
  <summary><strong>ECCV 2024</strong>（共筛选出 25 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 340 | [Vision-Language Action Knowledge Learning for Semantic-Aware Action Quality Assessment](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/05909.pdf) | Huangbiao Xu | Poster | 6 | 😾 | 
| 346 | [Meta-optimized Angular Margin Contrastive Framework for Video-Language Representation Learning](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/10330.pdf) | Thanh Thong Nguyen | Poster | 8 | 😾 | 
| 381 | [Textual Knowledge Matters: Cross-Modality Co-Teaching for Generalized Visual Class Discovery](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/06840.pdf) | Haiyang Zheng | Poster | 9 | 😾 | 
| 387 | [Masked Video and Body-worn IMU Autoencoder for Egocentric Action Recognition](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/02781.pdf) | Mingfang Zhang | Poster | 9 | 😾 | 
| 428 | [A Rotation-invariant Texture ViT for Fine-Grained Recognition of Esophageal Cancer Endoscopic Ultrasound Images](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/04582.pdf) | Tianyi Liu | Poster | 0 | 😾 | 
| 753 | [A Multimodal Benchmark Dataset and Model for Crop Disease Diagnosis](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/11606.pdf) | Xiang Liu | Poster | 3 | 😾 | 
| 762 | [Integration of Global and Local Representations for Fine-grained Cross-modal Alignment](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/10886.pdf) | Seungwan Jin | Poster | 1 | 😾 | 
| 781 | [PSALM: Pixelwise Segmentation with Large Multi-modal Model](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/04964.pdf) | Zheng Zhang | Poster | 32 | 😾 | 
| 792 | [LASS3D: Language-Assisted Semi-Supervised 3D Semantic Segmentation with Progressive Unreliable Data Exploitation](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/00378.pdf) | Jianan Li | Poster | 0 | 😾 | 
| 805 | [GTP-4o: Modality-prompted Heterogeneous Graph Learning for Omni-modal Biomedical Representation](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/00523.pdf) | Chenxin Li | Poster | 23 | 😾 | 
| 1082 | [Interaction-centric Spatio-Temporal Context Reasoning for Multi-Person Video HOI Recognition](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/04769.pdf) | Yisong Wang | Poster | 0 | 😾 | 
| 1098 | [Dual-stage Hyperspectral Image Classification Model with Spectral Supertoken](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/05047.pdf) | Peifu Liu | Poster | 4 | 😾 | 
| 1167 | [TIP: Tabular-Image Pre-training for Multimodal Classification with Incomplete Data](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/02403.pdf) | Siyi Du | Poster | 4 | 😾 | 
| 1370 | [Camera-LiDAR Cross-modality Gait Recognition](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/05072.pdf) | Wenxuan Guo | Poster | 4 | 😾 | 
| 1461 | [VSViG: Real-time Video-based Seizure Detection via Skeleton-based Spatiotemporal ViG](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/10768.pdf) | Yankun Xu | Poster | 2 | 😾 | 
| 1478 | [Label-anticipated Event Disentanglement for Audio-Visual Video Parsing](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/01492.pdf) | Jinxing Zhou | Poster | 15 | 😾 | 
| 1518 | [SCLIP: Rethinking Self-Attention for Dense Vision-Language Inference](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/03178.pdf) | Feng Wang | Poster | 64 | 😾 | 
| 1541 | [Test-Time Stain Adaptation with Diffusion Models for Histopathology Image Classification](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/05175.pdf) | Cheng-Chang Tsai | Poster | 3 | 😾 | 
| 1726 | [SAMFusion: Sensor-Adaptive Multimodal Fusion for 3D Object Detection in Adverse Weather](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/07882.pdf) | Edoardo Palladin | Poster | 6 | 😾 | 
| 1837 | [CoLeaF: A Contrastive-Collaborative Learning Framework for Weakly Supervised Audio-Visual Video Parsing](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/01653.pdf) | Faegheh Sardari | Poster | 2 | 😾 | 
| 1906 | [Pathology-knowledge Enhanced Multi-instance Prompt Learning for Few-shot Whole Slide Image Classification](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/01736.pdf) | Linhao Qu | Poster | 8 | 😾 | 
| 1924 | [Diagnosing and Re-learning for Balanced Multimodal Learning](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/08072.pdf) | Yake Wei | Poster | 5 | 😾 | 
| 2194 | [Upper-body Hierarchical Graph for Skeleton Based Emotion Recognition in Assistive Driving](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/03697.pdf) | Jiehui Wu | Poster | 0 | 😾 | 
| 2284 | [Norma: A Noise Robust Memory-Augmented Framework for Whole Slide Image Classification](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/06825.pdf) | Yu Bai | Poster | 1 | 😾 | 
| 2647 | [DGR-MIL: Exploring Diverse Global Representation in Multiple Instance Learning for Whole Slide Image Classification](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/05548.pdf) | Wenhui Zhu | Poster | 7 | 😾 | 
</details>
<!-- MM_CLASS_PAPERS_END -->

## ✅ Lung-Brain-AI-Paper-List
聚焦于**肺部和脑部医学场景**的论文
<!-- LB_PAPERS_START -->

<details>
  <summary><strong>NIPS 2025</strong>（共筛选出 34 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 05cVmYJJnb | [BrainMoE: Cognition Joint Embedding via Mixture-of-Expert Towards Robust Brain Foundation Model](https://openreview.net/forum?id=05cVmYJJnb) | Ziquan Wei | Poster |  | [Code😻](https://github.com/Chrisa142857/brain_moe) | 
| 6Q5WG84uOD | [THD-BAR: Topology Hierarchical Derived Brain Autoregressive Modeling for EEG Generic Representations](https://openreview.net/forum?id=6Q5WG84uOD) | Wenchao Yang | Poster |  | [Code😻](https://github.com/thdbar/THD-BAR) | 
| 6UAeCPQPwP | [Inference of Whole Brain Electrophysiological Networks Through Multimodal Integration of Simultaneous Scalp and Intracranial EEG](https://openreview.net/forum?id=6UAeCPQPwP) | Shihao Yang | Poster |  | 😾 | 
| 7yOl9qiLWd | [BrainODE: Neural Shape Dynamics for Age- and Disease-aware Brain Trajectories](https://openreview.net/forum?id=7yOl9qiLWd) | Wonjung Park | Poster |  | [Code😻](https://github.com/PWonjung/BrainODE) | 
| A9jXG3FUMT | [FAPEX: Fractional Amplitude-Phase Expressor for Robust Cross-Subject Seizure Prediction](https://openreview.net/forum?id=A9jXG3FUMT) | Ruizhe Zheng | Spotlight |  | 😾 | 
| BeSFxKX3Po | [Identifying interactions across brain areas while accounting for individual-neuron dynamics with a Transformer-based variational autoencoder](https://openreview.net/forum?id=BeSFxKX3Po) | Qi Xin | Poster |  | [Code😻](https://github.com/Qi-Xin/GLM-Transformer) | 
| CDQ0MI4rLw | [ZEBRA: Towards Zero-Shot Cross-Subject Generalization for Universal Brain Visual Decoding](https://openreview.net/forum?id=CDQ0MI4rLw) | Haonan Wang | Poster |  | [Code😻](https://github.com/xmed-lab/ZEBRA) | 
| DwFDfrPsm8 | [NOVA: A Benchmark for Rare Anomaly Localization and Clinical Reasoning in Brain MRI](https://openreview.net/forum?id=DwFDfrPsm8) | Cosmin I. Bercea | Oral |  | 😾 | 
| Fj2kyLs6G5 | [NeuroH-TGL: Neuro-Heterogeneity Guided Temporal Graph Learning Strategy for Brain Disease Diagnosis](https://openreview.net/forum?id=Fj2kyLs6G5) | Shengrong Li | Poster |  | 😾 | 
| K6ijewH34E | [Bridging Brains and Concepts: Interpretable Visual Decoding from fMRI with Semantic Bottlenecks](https://openreview.net/forum?id=K6ijewH34E) | Sara Cammarota | Poster |  | [Code😻](https://github.com/SaraCammarota/Bridging-Brains-and-Concepts) | 
| LDjBDk3Czb | [BaRISTA: Brain Scale Informed Spatiotemporal Representation of Human Intracranial Neural Activity](https://openreview.net/forum?id=LDjBDk3Czb) | Lucine L Oganesian | Poster |  | [Code😻](https://github.com/ShanechiLab/BaRISTA) | 
| NY3LzmUXl7 | [Riemannian Flow Matching for Brain Connectivity Matrices via Pullback Geometry](https://openreview.net/forum?id=NY3LzmUXl7) | Antoine Collas | Poster |  | [Code😻](https://github.com/facebookresearch/riemannian-fm) | 
| OM0Qkq9xtY | [Brain network science modelling of sparse neural networks enables Transformers and LLMs to perform as fully connected](https://openreview.net/forum?id=OM0Qkq9xtY) | Yingtao Zhang | Poster |  | [Code😻](https://github.com/biomedical-cybernetics/Cannistraci-Hebb-training) | 
| P4KjfMrVNU | [Hierarchical Information Aggregation for Incomplete Multimodal Alzheimer's Disease Diagnosis](https://openreview.net/forum?id=P4KjfMrVNU) | Chengliang Liu | Poster |  | 😾 | 
| RDdfQc5Ts1 | [AOR: Anatomical Ontology-Guided Reasoning for Medical Large Multimodal Model in Chest X-Ray Interpretation](https://openreview.net/forum?id=RDdfQc5Ts1) | Qingqiu Li | Poster |  | [Code😻](https://github.com/Liqq1/AOR) | 
| REIo9ZLSYo | [Brain-Inspired fMRI-to-Text Decoding via Incremental and Wrap-Up Language Modeling](https://openreview.net/forum?id=REIo9ZLSYo) | Wentao Lu | Spotlight |  | [Code😻](https://github.com/WENXUYUN/CogReader) | 
| Tt3XLyuDrE | [Transformer brain encoders explain human high-level visual responses](https://openreview.net/forum?id=Tt3XLyuDrE) | Hossein Adeli | Spotlight |  | [Code😻](https://github.com/Hosseinadeli/transformer_brain_encoder/) | 
| WQq5JPGQ0C | [RadZero: Similarity-Based Cross-Attention for Explainable Vision-Language Alignment in Chest X-ray with Zero-Shot Multi-Task Capability](https://openreview.net/forum?id=WQq5JPGQ0C) | Jonggwon Park | Poster |  | [Code😻](https://github.com/deepnoid-ai/RadZero) | 
| WtMuGdHvh6 | [S$^2$M-Former: Spiking Symmetric Mixing Branchformer for Brain Auditory Attention Detection](https://openreview.net/forum?id=WtMuGdHvh6) | Jiaqi Wang | Poster |  | [Code😻](https://github.com/JackieWang9811/S2M-Former) | 
| XmV7KRABBl | [EvoBrain: Dynamic Multi-Channel EEG Graph Modeling for Time-Evolving Brain Networks](https://openreview.net/forum?id=XmV7KRABBl) | Rikuto Kotoge | Spotlight |  | [Code😻](https://github.com/Kotoge/EvoBrain) | 
| YufSVJxDgt | [Inpainting the Neural Picture: Inferring Unrecorded Brain Area Dynamics from Multi-Animal Datasets](https://openreview.net/forum?id=YufSVJxDgt) | Ji Xia | Poster |  | 😾 | 
| agcXjEHmyW | [CSBrain: A Cross-scale Spatiotemporal Brain Foundation Model for EEG Decoding](https://openreview.net/forum?id=agcXjEHmyW) | Yuchen Zhou | Spotlight |  | [Code😻](https://github.com/yuchen2199/CSBrain) | 
| bDBxf9f5Li | [Care-PD: A Multi-Site Anonymized Clinical Dataset for Parkinson’s Disease Gait Assessment](https://openreview.net/forum?id=bDBxf9f5Li) | Vida Adeli | Poster |  | [Code😻](https://github.com/TaatiTeam/CARE-PD/) | 
| cjHQj0tCy6 | [BrainOmni: A Brain Foundation Model for Unified EEG and MEG Signals](https://openreview.net/forum?id=cjHQj0tCy6) | Qinfan Xiao | Poster |  | [Code😻](https://github.com/OpenTSLab/BrainOmni) | 
| fYSPRGmS6l | [MoRE-Brain: Routed Mixture of Experts for Interpretable and Generalizable Cross-Subject fMRI Visual Decoding](https://openreview.net/forum?id=fYSPRGmS6l) | YUXIANG WEI | Poster |  | [Code😻](https://github.com/yuxiangwei0808/MoRE-Brain) | 
| iQoZv77o3g | [Predicting Functional Brain Connectivity with Context-Aware Deep Neural Networks](https://openreview.net/forum?id=iQoZv77o3g) | Alexander Ratzan | Poster |  | [Code😻](https://github.com/neuroinfolab/GeneEx2Conn) | 
| kkhRTTmXFV | [Local-Global Coupling Spiking Graph Transformer for Brain Disorders Diagnosis from Two Perspectives](https://openreview.net/forum?id=kkhRTTmXFV) | Geng Zhang | Poster |  | 😾 | 
| koxXKW4FbR | [Listening to the Brain: Multi-Band sEEG Auditory Reconstruction via Dynamic Spatio-Temporal Hypergraphs](https://openreview.net/forum?id=koxXKW4FbR) | Xueyi Zhang | Poster |  | [Code😻](https://github.com/NeuroListen/NeuroListen) | 
| oCXyxhgCiZ | [BrainEC-LLM: Brain Effective Connectivity Estimation by Multiscale Mixing LLM](https://openreview.net/forum?id=oCXyxhgCiZ) | Wen Xiong | Poster |  | [Code😻](https://github.com/XiongWenXww/BrainEC-LLM) | 
| tPJg65EB7D | [Brain Harmony: A Multimodal Foundation Model Unifying Morphology and Function into 1D Tokens](https://openreview.net/forum?id=tPJg65EB7D) | Zijian Dong | Poster |  | [Code😻](https://github.com/hzlab/Brain-Harmony) | 
| tPqBnGwTwa | [Far from the Shallow: Brain-Predictive Reasoning Embedding through Residual Disentanglement](https://openreview.net/forum?id=tPqBnGwTwa) | Linyang He | Poster |  | 😾 | 
| vIDsnh0FWH | [AneuG-Flow: A Large-Scale Synthetic Dataset of Diverse Intracranial Aneurysm Geometries and Hemodynamics](https://openreview.net/forum?id=vIDsnh0FWH) | Wenhao Ding | Poster |  | [Code😻](https://github.com/yiyingsheng07/AneuG_CFD.git) | 
| xcw6UWNtFJ | [CXReasonBench: A Benchmark for Evaluating Structured Diagnostic Reasoning in Chest X-rays](https://openreview.net/forum?id=xcw6UWNtFJ) | Hyungyung Lee | Spotlight |  | [Code😻](https://github.com/ttumyche/CXReasonBench) | 
| ypPxYsmZPx | [DCA: Graph-Guided Deep Embedding Clustering for Brain Atlases](https://openreview.net/forum?id=ypPxYsmZPx) | Mo wang | Poster |  | [Code😻](https://github.com/ncclab-sustech/DCA) | 
</details>

<details>
  <summary><strong>MICCAI 2025</strong>（共筛选出 167 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
|  | A Learning Framework for Predicting CT-based PRM Biomarker from MRI Sequences in COPD |  |  |  | 😾 | 
|  | A Non-contrast Head CT Foundation Model for Comprehensive Neuro-Trauma Triage |  |  |  | 😾 | 
|  | A Unified Continuous Staging Framework for Alzheimer’s Disease and Lewy Body Dementia via Hierarchical Anatomical Features |  |  |  | 😾 | 
|  | Ada-FCN: Adaptive Frequency-Coupled Network for fMRI-Based Brain Disorder Classification |  |  |  | 😾 | 
|  | Adaptive Adversarial Data Augmentation with Trajectory Constraint for Alzheimer’s Disease Conversion Prediction |  |  |  | 😾 | 
|  | Adaptive Embedding for Long-Range High-Order Dependencies via Time-Varying Transformer on fMRI |  |  |  | 😾 | 
|  | Adaptive Graph Learning with Multi-Graph Convolutions for Brain Disorder Classification |  |  |  | 😾 | 
|  | Alzheimer’s Disease Recognition Based on Adaptive Graph Normalization Flow for Incomplete Multimodal Data Fusion |  |  |  | 😾 | 
|  | Anatomical Graph-based Multilevel Distillation for Robust Alzheimer’s Disease Diagnosis with Missing Modalities |  |  |  | 😾 | 
|  | Anatomy-Guided Multimodal Graph Networks for Alzheimer’s Disease: Integrative Analysis of Cross-Modal Brain Connectivity Signatures |  |  |  | 😾 | 
|  | Anatomy-based Self-supervised Pre-training for Scale-robust Hierarchical Representations in Chest X-rays |  |  |  | 😾 | 
|  | Asynchronous Multi-Modal Learning for Dynamic Risk Monitoring of Acute Respiratory Distress Syndrome in Intensive Care Units |  |  |  | 😾 | 
|  | Attention-Based Multimodal Deep Learning Model for Post-Stroke Motor Impairment Prediction |  |  |  | 😾 | 
|  | Attention-Guided Vector Quantized Variational Autoencoder for Brain Tumor Segmentation |  |  |  | 😾 | 
|  | Aβ-PET Pattern Prediction via Graph Reconstruction-Aware Fusion (GRAF) of Functional and Structural Networks |  |  |  | 😾 | 
|  | BREA-Depth: Bronchoscopy Realistic Airway-geometric Depth Estimation |  |  |  | 😾 | 
|  | BiSCoT: Behavior-Informed Subgroup-Consistent Connectome Template for Interpretable Brain Network Analysis |  |  |  | 😾 | 
|  | Blood Pressure Assisted Cerebral Microbleed Segmentation via Meta-matching |  |  |  | 😾 | 
|  | BraTS-UMamba: Adaptive Mamba UNet with Dual-Band Frequency based Feature Enhancement for Brain Tumor Segmentation |  |  |  | 😾 | 
|  | Brain Wiring Knowledge Graph Reasoning: A Region Embedding Approach for Logical Neuronal Relation Inference |  |  |  | 😾 | 
|  | Brain activation mapping based on Regional Synchronization of fMRI signals embedded in Graph Eigenmodes |  |  |  | 😾 | 
|  | Brain-Environment Cross-Attention (BECA) Meta-Matching: A New Perspective of Brain Connectome Zero-Shot Learning |  |  |  | 😾 | 
|  | Brain-Heart-Gut Guided Multi-Constraint Knowledge Distillation for Early Alzheimer’s Disease Diagnosis |  |  |  | 😾 | 
|  | BrainAlign: EEG-Vision Alignment via Frequency-Aware Temporal Encoder and Differentiable Cluster Assigner |  |  |  | 😾 | 
|  | BrainMT: A Hybrid Mamba-Transformer Architecture for Modeling Long-Range Dependencies in Functional MRI Data |  |  |  | 😾 | 
|  | BrainPrompt: Domain Adaptation with Prompt Learning for Multi-site Brain Network Analysis |  |  |  | 😾 | 
|  | BrainPrompt: Multi-Level Brain Prompt Enhancement for Neurological Condition Identification |  |  |  | 😾 | 
|  | Bridging the Gap in Missing Modalities: Leveraging Knowledge Distillation and Style Matching for Brain Tumor Segmentation |  |  |  | 😾 | 
|  | CBrain: Cross-Modal Learning for Brain Vigilance Detection in Resting-State fMRI |  |  |  | 😾 | 
|  | CLIP-DSA: Textual Knowledge-Guided Cerebrovascular Diseases Recognition in Multi-View Digital Subtraction Angiography |  |  |  | 😾 | 
|  | CT-Based Hippocampus Segmentation with Dual-Decoder Network (HDD-Net) |  |  |  | 😾 | 
|  | CXR-CML: Improved zero-shot classification of long-tailed multi-label diseases in Chest X-Rays |  |  |  | 😾 | 
|  | CXR-TFT: Multi-Modal Temporal Fusion Transformer for Predicting Chest X-ray Trajectories |  |  |  | 😾 | 
|  | Cerebrovascular Diseases Screening from Color Fundus Photography via Cross-View Fusion and Graph-Based Discrimination |  |  |  | 😾 | 
|  | ClinGRAD: Clinically-Guided Genomics and Radiomics Interpretable GNN for Dementia Diagnosis |  |  |  | 😾 | 
|  | Clinical Data-Driven Retrieval-Augmented Model for Lung Nodule Malignancy Prediction |  |  |  | 😾 | 
|  | Clinical Prior Guided Cross-Modal Hierarchical Fusion for Histological Subtyping of Lung Cancer in CT Scans |  |  |  | 😾 | 
|  | CoC: Chain-of-Cancer based on Cross-Modal Autoregressive Traction for Survival Prediction |  |  |  | 😾 | 
|  | CoCa-CXR: Contrastive Captioners Learn Strong Temporal Structures for Chest X-Ray Vision-Language Understanding |  |  |  | 😾 | 
|  | Collateral Circulation guided Multi-modality Fusion Network for Postoperative Infarct Prediction |  |  |  | 😾 | 
|  | Conditional Graph Diffusion with Topological Constraints for Brain Network Generation |  |  |  | 😾 | 
|  | Controllable Flow Matching for 3D Contrast-Enhanced Brain MRI Synthesis from Non-Contrast Scans |  |  |  | 😾 | 
|  | Cross-Modal Brain Graph Transformer via Function-Structure Connectivity Network for Brain Disease Diagnosis |  |  |  | 😾 | 
|  | Cross-Modal CXR-CTPA Knowledge Distillation using latent diffusion priors towards CXR Pulmonary Embolism Diagnosis |  |  |  | 😾 | 
|  | Cross-Modal Graph Learning for Perivascular Spaces Segmentation |  |  |  | 😾 | 
|  | Cross-Modality Masked Learning for Survival Prediction in ICI Treated NSCLC Patients |  |  |  | 😾 | 
|  | C²MAOT: Cross-modal Complementary Masked Autoencoder with Optimal Transport for Cancer Segmentation in PET-CT Images |  |  |  | 😾 | 
|  | D3M: Deformation-Driven Diffusion Model for Synthesis of Contrast-Enhanced MRI with Brain Tumors |  |  |  | 😾 | 
|  | DC-Seg: Disentangled Contrastive Learning for Brain Tumor Segmentation with Missing Modalities |  |  |  | 😾 | 
|  | DEFUSE-MS: Deformation Field-Guided Spatiotemporal Graph-Based Framework for Multiple Sclerosis New Lesion Detection |  |  |  | 😾 | 
|  | DGM: Disentangled Generative Model for Detecting AD Individualized Pathological Changes via Pseudo-Healthy Synthesis |  |  |  | 😾 | 
|  | DHGFormer: Dynamic Hierarchical Graph Transformer for Disorder Brain Disease Diagnosis |  |  |  | 😾 | 
|  | DISCLOSE the Neurodegeneration Dynamics: Individualized ODE Discovery for Alzheimer’s Disease Precision Medicine |  |  |  | 😾 | 
|  | Deep Knowledge-Infused Transformer for NSCLC Lymph Node Station Metastasis Prediction: Development of an AI-Powered Intraoperative Decision System |  |  |  | 😾 | 
|  | Disentangled and Interpretable Multimodal Attention Fusion for Cancer Survival Prediction |  |  |  | 😾 | 
|  | Distribution-Guided Multi-Tracer Brain PET Synthesis from Structural MRI with Class-Conditioned Weighted Diffusion |  |  |  | 😾 | 
|  | Domain Generalization for Pulmonary Nodule Detection via Distributionally-Regularized Mamba |  |  |  | 😾 | 
|  | Domain-Adaptive Diagnosis of Lewy Body Disease with Transferability Aware Transformer |  |  |  | 😾 | 
|  | Domain-Agnostic Stroke Lesion Segmentation Using Physics-Constrained Synthetic Data |  |  |  | 😾 | 
|  | Dual-Stream Multi-Band Fusion Network for Dynamic Functional Connectivity Analysis in Brain Disorder Classification |  |  |  | 😾 | 
|  | Dynamic Function-Structure Connectivity Coupling for Predicting Progression Trajectories in Neurocognitive Decline |  |  |  | 😾 | 
|  | EEG-DINO: Learning EEG Foundation Models via Hierarchical Self-Distillation |  |  |  | 😾 | 
|  | EFMS-Net: Efficient Frequency-Enhanced Multi-Scale Network for Ischemic Stroke Segmentation |  |  |  | 😾 | 
|  | Edge-Aware Hierarchical Graph Transformer to Decode Brain Arterial Network |  |  |  | 😾 | 
|  | Explainable ADHD Diagnostic Framework Using Weakly-Supervised Action Recognition |  |  |  | 😾 | 
|  | FViM: Frequency Vision Mamba for Label-Free Cell Death Pathway Prediction in Lung Cancer Chemotherapy |  |  |  | 😾 | 
|  | Feature Copy-Paste Network for Lung Cancer EGFR Mutation Status Prediction in CT images |  |  |  | 😾 | 
|  | FedAMM: Federated Learning for Brain Tumor Segmentation with Arbitrary Missing Modalities |  |  |  | 😾 | 
|  | Flexibly Distilled 3D Rectified Flow with Anatomical Constraints for Developmental Infant Brain MRI Prediction |  |  |  | 😾 | 
|  | FoundBioNet: A Foundation-Based Model for IDH Genotyping of Glioma from Multi-Parametric MRI |  |  |  | 😾 | 
|  | Foundation-Model-Boosted Multimodal Learning for fMRI-based Neuropathic Pain Drug Response Prediction |  |  |  | 😾 | 
|  | GL-LCM: Global-Local Latent Consistency Models for Fast High-Resolution Bone Suppression in Chest X-Ray Images |  |  |  | 😾 | 
|  | GeneMorphFormer: Transformer-Driven Cross-Scale Mapping from Gene Expression to Cortical Morphology |  |  |  | 😾 | 
|  | Generative Unsupervised Anomaly Detection with Coarse-Fine Ensemble for Workload Reduction in 3D Non-contrast Brain CT of Emergency Room |  |  |  | 😾 | 
|  | Graph Disentanglement Learning for fMRI Analysis: Decoupling Disease, Covariates, and Individual Variability |  |  |  | 😾 | 
|  | Graph-PAVNet: A Graph-Based Learning Framework for Pulmonary Artery and Vein Separation Using Multimodal Feature Sampling |  |  |  | 😾 | 
|  | Harnessing EHRs for Diffusion-based Anomaly Detection on Chest X-rays |  |  |  | 😾 | 
|  | Heterogeneous Masked Attention-Guided Path Convolution for Functional Brain Network Analysis |  |  |  | 😾 | 
|  | Hierarchical Anatomy-Aware Guidance for Brain Tissue Microstructure Reconstruction from T1-weighted MRI |  |  |  | 😾 | 
|  | Hyperbolic Kernel GCN with Structure-Function Connectivity Coupling for Neurocognitive Impairment Analysis |  |  |  | 😾 | 
|  | Hypergraph Tversky-Aware Domain Incremental Learning for Brain Tumor Segmentation with Missing Modalities |  |  |  | 😾 | 
|  | Hypergraph-Guided Federated Distillation Learning for Efficient and Robust Multi-Center fMRI Data Analysis |  |  |  | 😾 | 
|  | IM-Fuse: A Mamba-based Fusion Block for Brain Tumor Segmentation with Incomplete Modalities |  |  |  | 😾 | 
|  | IP-CRR: Information Pursuit for Interpretable Classification of Chest Radiology Reports |  |  |  | 😾 | 
|  | Improving Autism Detection with Multimodal Behavioral Analysis |  |  |  | 😾 | 
|  | Integrating meta-analysis in multi-modal brain studies with graph-based attention transformer |  |  |  | 😾 | 
|  | Investigating Voxel-level Brain Age Prediction as a Pretext Task for Brain MRI Segmentation |  |  |  | 😾 | 
|  | Knowledge Tree Driven Contextualized Instruction Tuning of Foundation Models for Epilepsy Drug Recommendation |  |  |  | 😾 | 
|  | LG-DBGL: Lateralization-Guided Dissociative Brain Graph Learning for Alzheimer’s Disease Identification |  |  |  | 😾 | 
|  | LLM-Powered Cross-Modal Alignment for Explainable Seizure Detection from EEG |  |  |  | 😾 | 
|  | Learning 3D Medical Image Models From Brain Functional Connectivity Network Supervision For Mental Disorder Diagnosis |  |  |  | 😾 | 
|  | Learning Explainable Imaging-Genetics Associations Related to a Neurological Disorder |  |  |  | 😾 | 
|  | Learning with Explicit Topological Priors for Chest X-ray Rib Segmentation |  |  |  | 😾 | 
|  | Lesion-centered vision transformer for stroke outcome prediction from image and clinical data |  |  |  | 😾 | 
|  | MAGNET-AD: Multitask Spatiotemporal GNN for Interpretable Prediction of PACC and Conversion Time in Preclinical Alzheimer |  |  |  | 😾 | 
|  | MDAA-Diff: CT-Guided Multi-Dose Adaptive Attention Diffusion Model for PET Denoising |  |  |  | 😾 | 
|  | MG-UNet: A Memory-Guided UNet for Lesion Segmentation in Chest Images |  |  |  | 😾 | 
|  | MGG-Net: A Multi-Modal Feature Extraction and Global-Aware Feature Graph-Based Deep Learning Network for MGMT Status Classification in Glioma |  |  |  | 😾 | 
|  | MIBF-Net: Multi-modal Information Balanced Fusion Network for Clinical Diagnosis via Patient Narratives and Lesion Image |  |  |  | 😾 | 
|  | MMBNA: Masked Multiview Brain Network Analysis via Disentangling for Alzheimer’s Early Diagnosis with fMRI |  |  |  | 😾 | 
|  | MNM: Multi-level Neuroimaging Meta-analysis with Hyperbolic Brain-Text Representations |  |  |  | 😾 | 
|  | MSGFlowNet: Learning Effective Connectivity Network based on Sparse Generative Flow Network from fMRI and EEG Data |  |  |  | 😾 | 
|  | MVP-LLMs: Optimizing Intervention Timing and Subsequent Decision Support for Mechanical Ventilation Parameter Control Using Large Language Models |  |  |  | 😾 | 
|  | Marker-less Head Pose Tracking for Image-guided Cerebral Artery Navigation |  |  |  | 😾 | 
|  | Masked Contrastive Language-Image Modeling For Brain Segmentation |  |  |  | 😾 | 
|  | Meta-analysis guided multi-task graph transformer network for diagnosis of neurological disease and cognitive deficits |  |  |  | 😾 | 
|  | MindLink: Subject-agnostic Cross-Subject Brain Decoding Framework |  |  |  | 😾 | 
|  | Multi-Modal Graph-Based Machine Learning for Predicting Surgical Outcome in Epilepsy Patients |  |  |  | 😾 | 
|  | Multi-Sensory Cognitive Computing for Learning Population-level Brain Connectivity |  |  |  | 😾 | 
|  | Multi-Spatial Granger Causality Features Fusion Network for Alzheimer’s Disease Classification |  |  |  | 😾 | 
|  | Multi-subject Orthogonal Sparse Matrix Decomposition Method for Extracting Individual Brain Functional Networks |  |  |  | 😾 | 
|  | Multi-view Graph Contrastive Learning with Dynamic Self-aware and Cross-sample Topology Augmentation for Brain Disorder Diagnosis |  |  |  | 😾 | 
|  | MultiTransAD: Cross-Sequence Translation-Driven Anomaly Detection in Multi-Sequence Brain MRI |  |  |  | 😾 | 
|  | Multistage Alignment and Fusion for Multimodal Multiclass Alzheimer’s Disease Diagnosis |  |  |  | 😾 | 
|  | MvHo-IB: Multi-View Higher-Order Information Bottleneck for Brain Disorder Diagnosis |  |  |  | 😾 | 
|  | Neighborhood-Consistent Binary Transformation for Domain-Invariant Chest X-ray Diagnosis |  |  |  | 😾 | 
|  | Neuro-AMS: Neuro-informed Age-aware and Medical Knowledge-integrated Strategy for Diagnosis of Multiple Brain Disorders |  |  |  | 😾 | 
|  | NeuroXVocal: Detection and Explanation of Alzheimer’s Disease through Non-invasive Analysis of Picture-prompted Speech |  |  |  | 😾 | 
|  | New Multiple Sclerosis Lesion Segmentation via Calibrated Inter-patch Blending |  |  |  | 😾 | 
|  | Noise-Controllable Complex-Valued Diffusion Model for k-Space Data of Hyperpolarized 129Xe Lung MRI Generation |  |  |  | 😾 | 
|  | Non-Invasive TB Detection using Acoustic and Semantic Features from Cough Sounds |  |  |  | 😾 | 
|  | Oblique Genomics Mixture of Experts: Prediction of Brain Disorder With Aging-Related Changes of Brain’s Structural Connectivity Under Genomic Influences |  |  |  | 😾 | 
|  | PMIL: Prompt enhanced Multimodal Integrative analysis of fMRI combining functional connectivity and temporal Latency |  |  |  | 😾 | 
|  | PRGNN: Pyramidal Region Graph Neural Network for Region-Based Brain PET Classification |  |  |  | 😾 | 
|  | Path Signature Features Revealed SSRI-Induced White Matter Morphological Reorganization in Depressions |  |  |  | 😾 | 
|  | Phenotype Representation and Analysis via Discriminative Atypicality (PRADA) to capture the structural heterogeneity of Autism Spectrum Disorder |  |  |  | 😾 | 
|  | Phrase-grounded Fact-checking for Automatically Generated Chest X-ray Reports |  |  |  | 😾 | 
|  | Predicting Longitudinal Brain Development via Implicit Neural Representations |  |  |  | 😾 | 
|  | Pretraining on Chronic Lung Inflammatory Disease Datasets to Enhance Indeterminant Lung Cancer Classification using Masked Autoencoders |  |  |  | 😾 | 
|  | Prior-guided Prototype Aggregation Learning for Alzheimer’s Disease Diagnosis |  |  |  | 😾 | 
|  | Privacy Preserving Chest X-ray Classification in Latent Space with Homomorphically Encrypted Neural Inference |  |  |  | 😾 | 
|  | Progression-Aware Generative Model Enhancing Baseline Visit Prediction of Early Alzheimer’s Disease |  |  |  | 😾 | 
|  | RSAD: Region-Specific Anomaly Detection in fMRI for Disease Diagnosis |  |  |  | 😾 | 
|  | RadioFormer: Integrating Radiologist Inductive Bias for Tumor Classification on Multi-Sequence MR Images |  |  |  | 😾 | 
|  | Reflect: Rectified Flows for Efficient Brain Anomaly Correction Transport |  |  |  | 😾 | 
|  | SMF-Net: Unlocking Multimodal Insights for Enhanced Stroke Lesion Segmentation |  |  |  | 😾 | 
|  | SPEC-CXR: Advancing Clinical Safety through Entity-Level Performance Evaluation of Chest X-ray Report Generation |  |  |  | 😾 | 
|  | SPromptGL: Semantic Prompt Guided Graph Learning for Multi-modal Brain Disease |  |  |  | 😾 | 
|  | SSPNet: Towards Feasible Spatio-Spectral Portraits-Based Deep Learning Framework for Neurodegenerative Disease Multi-Classification |  |  |  | 😾 | 
|  | Self-adaptive Vision-Language Model for 3D Segmentation of Pulmonary Artery and Vein |  |  |  | 😾 | 
|  | Semantic-Aware Chest X-ray Report Generation with Domain-Specific Lexicon and Diversity-Controlled Retrieval |  |  |  | 😾 | 
|  | ShareLink: Neuro-Inspired EEG-based Cross-Subject Emotion Recognition via Shared Bi-hemisphere |  |  |  | 😾 | 
|  | Small Lesions-aware Bidirectional Multimodal Multiscale Fusion Network for Lung Disease Classification |  |  |  | 😾 | 
|  | Sparsely Labeled fMRI Data Denoising with Meta-Learning-Based Semi-Supervised Domain Adaptation |  |  |  | 😾 | 
|  | Spatial Prior-Guided Boundary and Region-Aware 2D Lesion Segmentation in Neonatal Hypoxic Ischemic Encephalopathy |  |  |  | 😾 | 
|  | Spherical Diffusion Process for Score-Guided Cortical Correspondence via Spectral Attention |  |  |  | 😾 | 
|  | Structure-Aware Cross-Modal Prompt Tuning for Autonomous Bronchoscopic Navigation |  |  |  | 😾 | 
|  | StyleGAN-based Brain MRI Anomaly Detection via Latent Code Retrieval and Partial Swap |  |  |  | 😾 | 
|  | Surface-based Multi-Axis Longitudinal Disentanglement Using Contrastive Learning for Alzheimer’s Disease |  |  |  | 😾 | 
|  | T&F-DFC FusionNet: Time&Frequency-Dynamic Functional Connectivity Fusion Network for ADHD Diagnosis in Children based on fNIRS |  |  |  | 😾 | 
|  | TESLA: Test-time Reference-free Through-plane Super-resolution for Multi-contrast Brain MRI |  |  |  | 😾 | 
|  | TRRG: Towards Truthful Radiology Report Generation With Cross-modal Disease Clue Enhanced Large Language Models |  |  |  | 😾 | 
|  | Task-aligned fMRI Generation Model for Brain Disorder Diagnosis |  |  |  | 😾 | 
|  | TemSAM: Temporal-aware Segment Anything Model for Cerebrovascular Segmentation in Digital Subtraction Angiography Sequences |  |  |  | 😾 | 
|  | Tetra-orientated Mamba with T2-FLAIR Mismatch Features for Glioma Segmentation, IDH Genotyping, and Grading |  |  |  | 😾 | 
|  | TextBraTS: Text-Guided Volumetric Brain Tumor Segmentation with Innovative Dataset Development and Fusion Module Exploration |  |  |  | 😾 | 
|  | This EEG Looks Like These EEGs: Interpretable Interictal Epileptiform Discharge Detection With ProtoEEG-kNN |  |  |  | 😾 | 
|  | Thread the Needle: Genomics-guided Prompt-bridged Attention Model for Survival Prediction of Glioma based on MRI Images |  |  |  | 😾 | 
|  | Two-Stage Generative Model for Intracranial Aneurysm Meshes with Morphological Marker Conditioning |  |  |  | 😾 | 
|  | Uncertainty-Aware Multimodal MRI Fusion for HIV-Associated Asymptomatic Neurocognitive Impairment Prediction |  |  |  | 😾 | 
|  | UniCross: Balanced Multimodal Learning for Alzheimer’s Disease Diagnosis by Uni-modal Separation and Metadata-guided Cross-modal Interaction |  |  |  | 😾 | 
|  | UniSegDiff: Boosting Unified Lesion Segmentation via a Staged Diffusion Model |  |  |  | 😾 | 
|  | Unpaired Multi-Site Brain MRI Harmonization with Image Style-Guided Latent Diffusion |  |  |  | 😾 | 
|  | Unraveling Brainstem Deformations in Joubert Syndrome: A Statistical Shape Analysis of MRI-Derived Structures |  |  |  | 😾 | 
|  | VT-SNN: Variable Time-step Spiking Neural Network Based on Uncertainty Measure and Its Application in Brain Disease Diagnosis |  |  |  | 😾 | 
|  | X-SiT: Inherently Interpretable Surface Vision Transformers for Dementia Diagnosis |  |  |  | 😾 | 
|  | tHPM-LDM: Integrating Individual Historical Record with Population Memory in Latent Diffusion-based Glaucoma Forecasting |  |  |  | 😾 | 
</details>

<details>
  <summary><strong>ISBI 2025</strong>（共筛选出 99 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
|  | 3D Multi-Modal Feature Interaction Fusion Network for Early Alzheimer's Disease Diagnosis |  |  |  | 😾 | 
|  | 3D Semantic Segmentation of Airway Abnormalities on UTE-MRI with Reinforcement Learning on Deep Supervision |  |  |  | 😾 | 
|  | A Comparison of Object Detection and Phrase Grounding Models in Chest X-Ray Abnormality Localization Using Eye-Tracking Data |  |  |  | 😾 | 
|  | A Semi-Supervised Learning Framework with Cross-Magnification Attention for Glioma Mitosis Classification |  |  |  | 😾 | 
|  | AEPL: Automated and Editable Prompt Learning for Brain Tumor Segmentation |  |  |  | 😾 | 
|  | AGE2HIE: Transfer Learning from Brain Age to Predicting Neurocognitive Outcome for Infant Brain Injury |  |  |  | 😾 | 
|  | Adaptive Cross-Attention for Robust Lung Segmentation with Noisy Labels |  |  |  | 😾 | 
|  | Age Knowledge Guided CNN-Transformer Network for Alzheimer's Disease Diagnosis |  |  |  | 😾 | 
|  | Assessing the Use of Diffusion Models for Motion Artifact Correction in Brain MRI |  |  |  | 😾 | 
|  | Augmenting Spline Interpolated Dementia EEG Data with Deep Learning for Improved Multi-Centric Data Harmonization |  |  |  | 😾 | 
|  | Automatic Classification and Explanation of Abnormal EEGs Using Signal Images and Spectrograms |  |  |  | 😾 | 
|  | Automatic Generation of Brain Tumor Diagnostic Reports from Multimodality MRI Using Large Language Models |  |  |  | 😾 | 
|  | BMENet: 3D Brain MRI Enhancement Against Various Degradations with Coarse to Fine Network |  |  |  | 😾 | 
|  | BOrg: A Brain Organoid-Based Mitosis Dataset for Automatic Analysis of Brain Diseases |  |  |  | 😾 | 
|  | Brain Network Alignment Using Structural and Functional Connectivity With Anatomical Constraints |  |  |  | 😾 | 
|  | Brain-Adapter: Enhancing Neurological Disorder Analysis with Adapter-Tuning Multimodal Large Language Models |  |  |  | 😾 | 
|  | Braineditor: Structure-Disentangled Brain MRI Synthesis via Natural Language Prompted Diffusion Image Editing |  |  |  | 😾 | 
|  | Breaking the Disconnection Dilemma in Class-Imbalanced Cerebrovascular Segmentation |  |  |  | 😾 | 
|  | Bridging the Gap Between Conventional and CNN-Based Registration: Application to Mouse Brain Morphometry |  |  |  | 😾 | 
|  | CFBM: Cubic Filtered Brain Map Creation Method and Models for Eeg-Based Emotion Recognition |  |  |  | 😾 | 
|  | CLISC: Bridging Clip and SAM by Enhanced CAM for Unsupervised Brain Tumor Segmentation |  |  |  | 😾 | 
|  | COCTA: A Cortex Vessel Segmentation Benchmark Using Optical Coherence Tomography Angiography |  |  |  | 😾 | 
|  | CSF-NET: Cross-Modal Spatiotemporal Fusion Network for Pulmonary Nodule Malignancy Predicting |  |  |  | 😾 | 
|  | CT-AGRG: Automated Abnormality-Guided Report Generation from 3D Chest CT Volumes |  |  |  | 😾 | 
|  | Causal Modeling of FMRI Time-Series for Interpretable Autism Spectrum Disorder Classification |  |  |  | 😾 | 
|  | Cell Image Classification with Polygon-Guided Extraction for Glioma Mitosis Detection |  |  |  | 😾 | 
|  | Classiffication of Mild Cognitive Impairment Based on Dynamic Functional Connectivity using Spatio-Temporal Transformer |  |  |  | 😾 | 
|  | Conditional Generative Models for Contrast-Enhanced Synthesis of T1W and T1 Maps in Brain MRI |  |  |  | 😾 | 
|  | Connectome-GTC: A Unified Framework for Brain Functional and Structural Connectomes Generation, Translation, and Classification |  |  |  | 😾 | 
|  | Controllable Latent Diffusion-Based 3D Brain Tumor Segmentation: With Synthetic Label Generation and Detailed Variance Map |  |  |  | 😾 | 
|  | Curriculum Learning for Few-Shot Domain Adaptation in CT-Based Airway Tree Segmentation |  |  |  | 😾 | 
|  | Deep Neural Network for Lung Adenocarcinoma Subtype from Multimodal Fusion of Imaging and Clinical Data |  |  |  | 😾 | 
|  | Detecting Pain During Cognitive Tasks via Visibility Graphs of Cortical Signals |  |  |  | 😾 | 
|  | Diff-Lung: Diffusion-Based Texture Synthesis for Enhanced Pathological Tissue Segmentation in Lung CT Scans |  |  |  | 😾 | 
|  | DiffKAN-Inpainting: KAN-Based Diffusion Model for Brain Tumor Inpainting |  |  |  | 😾 | 
|  | Dual Multi-Atlas Representation Alignment for Brain Disorder Diagnosis Using Morphological Connectome |  |  |  | 😾 | 
|  | EMViR - Electron Microscopy Image Dataset Of Labelled Respiratory Viruses For Deep Learning Model Development |  |  |  | 😾 | 
|  | Efficient 4D fMRI ASD Classification using Spatial-Temporal-Omics-Based Learning Framework |  |  |  | 😾 | 
|  | Enhanced Mitotic Figure Detection in Glioma Using Super-Resolution Images and High-Frequency Content Maps |  |  |  | 😾 | 
|  | Enhancing Dementia Detection: Augmenting the Face Mesh |  |  |  | 😾 | 
|  | ExpTopo: Explicit Topological Modeling for Bronchus Segmentation |  |  |  | 😾 | 
|  | Explainable Brain Age Gap Prediction in Neurodegenerative Conditions Using Covariance Neural Networks |  |  |  | 😾 | 
|  | Explainsegnet: Interpretable Segmentation for Alzheimer's Diagnosis |  |  |  | 😾 | 
|  | Exploring Test Time Adaptation for Subcortical Segmentation of the Fetal Brain in 3D Ultrasound |  |  |  | 😾 | 
|  | Functional-Positional Context Embedding in Graph Neural Networks for Improved Parkinson's Disease Diagnosis |  |  |  | 😾 | 
|  | Fusion of Dwi Image and Clinical Variables for Stroke Outcome Prediction Using Tabular Transformer |  |  |  | 😾 | 
|  | GL-ICNN: An End-To-End Interpretable Convolutional Neural Network for the Diagnosis and Prediction of Alzheimer's Disease |  |  |  | 😾 | 
|  | Ga-Shape: A Dual-Path Framework Guided by Gestational Age and Local Shape Descriptors for Fetal Brain MRI Segmentation |  |  |  | 😾 | 
|  | Generalized Cross-Domain Multi-Label Few-Shot Learning for Chest X-Rays |  |  |  | 😾 | 
|  | Genetics Encoded Joint Embedding of Multimodal Connectomes with Explainable Graph Neural Network for Schizophrenia Classification |  |  |  | 😾 | 
|  | Hybridization of Attention UNet with Repeated Atrous Spatial Pyramid Pooling for Improved Brain Tumor Segmentation |  |  |  | 😾 | 
|  | ITCFN: Incomplete Triple-Modal Co-Attention Fusion Network for Mild Cognitive Impairment Conversion Prediction |  |  |  | 😾 | 
|  | Identifying Brain Strokes by Image Analysis Using Transfer Learning |  |  |  | 😾 | 
|  | Improved Brain Disorder Classification in RS-FMRI via Inter-Individual Variability Characterization: A Self-Supervised Learning Approach |  |  |  | 😾 | 
|  | Integrating Event-Based and Biophysical Models to Predict Individual Tau Progression in Alzheimer's Disease |  |  |  | 😾 | 
|  | Joint Source Decomposition of Concurrent EEG-FMRI Data in Epilepsy and Control Groups |  |  |  | 😾 | 
|  | Linking Multi-Scale Brain Connectivity with Vigilance, Working Memory, and Behavior in Adolescents |  |  |  | 😾 | 
|  | Longitudinal Image Synthesis for Early Brain Development with Age Awareness |  |  |  | 😾 | 
|  | Medvisiochat: A Multimodal Large Language Model Framework for Interpretable Diagnosis with Visual Grounding in CXRs |  |  |  | 😾 | 
|  | Misalignment-Aware MRI-to-CT Synthesis for Lung Segmentation on MRI |  |  |  | 😾 | 
|  | MoAR-Diff: Controlled Diffusion Model for Infant Brain MR Motion Artifact Reduction |  |  |  | 😾 | 
|  | Multi-Modal Fusion Network Integrating Imaging and Clinical Tabular Data for Alzheimer's Disease Classification |  |  |  | 😾 | 
|  | Multi-Scale Time-Frequency Fusion Intelligent Diagnosis Model for Children with ADHD Based on fNIRS |  |  |  | 😾 | 
|  | Multi-VentNet: Multimodal Fusion Framework for Predicting Need for Mechanical Ventilation |  |  |  | 😾 | 
|  | Multi-View Transformers for Airway-to-Lung Ratio Inference on Cardiac CT Scans: The C4R Study |  |  |  | 😾 | 
|  | Multicenter Radiomic Feature-Based Prediction of PD-L1 Status in Lung Cancer Using Multimodal PET/CT |  |  |  | 😾 | 
|  | Multimodal Learning for Lung Segmentation: Enhancing UTE MRI Segmentation with CT Datasets |  |  |  | 😾 | 
|  | Multiset FMRI Data Analysis with Subject Group Information using Structured Dictionary Learning |  |  |  | 😾 | 
|  | Non-Invasive Molecular Classification of Gliomas Using Explainable ai and Radiomic Features from Multi-Sequence MRI |  |  |  | 😾 | 
|  | Omnimamba4D: Spatio-Temporal Mamba for Longitudinal CT Lesion Segmentation |  |  |  | 😾 | 
|  | Overall Survival Prediction of Brain Tumor Patients with Multimodal MRI using Swin Unetr |  |  |  | 😾 | 
|  | Physics-Informed Autoencoder for DSC-MRI Perfusion Post-Processing: Application to Glioma Grading |  |  |  | 😾 | 
|  | Predicting Future Cognitive Development Using Past Brain FMRI with Spatiotemporal Graph Transformers Guided by Prior Information |  |  |  | 😾 | 
|  | Predicting Intelligence Profile and Brain Age with Single- and Dual-Channel Cnns: A Study Based on Human Connectome Projects |  |  |  | 😾 | 
|  | Pulmofusion: Advancing Pulmonary Health with Efficient Multi-Modal Fusion |  |  |  | 😾 | 
|  | Quantifying White Matter Hyperintensities: Predicting Periventricular Fazekas Scores with Uncertainty Estimation |  |  |  | 😾 | 
|  | Real-Time Eeg-Based Epileptic Seizure Detection by One-Class Classification using CNN-LSTM and STFT |  |  |  | 😾 | 
|  | Revealing Brain and Spinal Cord Functional Networks by Block-Structured Graph Laplacian Mixture Models |  |  |  | 😾 | 
|  | Risk-RAM: An Interpretable Deep Survival Prediction Model for COVID-19 Prognosis Using Chest X-Ray Images |  |  |  | 😾 | 
|  | SFECNet: Fusion of Structural, Functional and Effective Connectivity Network for Epilepsy Classification |  |  |  | 😾 | 
|  | SegCSR: Weakly-Supervised Cortical Surfaces Reconstruction from Brain Ribbon Segmentations |  |  |  | 😾 | 
|  | Self-Supervised Graph Transformer with Contrastive Learning for Brain Connectivity Analysis Towards Improving Autism Detection |  |  |  | 😾 | 
|  | Self-Supervised Multi-Modal Learning for Accurate MRI Multiple Sclerosis Segmentation |  |  |  | 😾 | 
|  | State-of-the-Art Stroke Lesion Segmentation at 1/1000th of Parameters |  |  |  | 😾 | 
|  | Stroke Lesion Segmentation Using Multi-Stage Cross-Scale Attention |  |  |  | 😾 | 
|  | Supervised Learning Assisted by Text-Guided Attention for Ards Prediction |  |  |  | 😾 | 
|  | TX-LLaVA: Large Language and Vision Assistant for Temporal Changes in Chest X-Rays |  |  |  | 😾 | 
|  | Temporal-Focal Attention on EEG for Cross-Patient Epileptic Seizure Detection |  |  |  | 😾 | 
|  | Topology-Aware Graph Augmentation for Predicting Clinical Trajectories in Neurocognitive Disorders |  |  |  | 😾 | 
|  | Train for Stain: Adapting for Diverse H&E Staining Profiles Across Centers in Classification of Mitotic Figures (Glioma-MDC 2025) |  |  |  | 😾 | 
|  | Ultrasound Image Synthesis Using Generative AI for Lung Consolidation Detection |  |  |  | 😾 | 
|  | Understanding the Spreading Mechanism of Tau Propagation in Alzheimers Disease Through a Multi-Layer Transport |  |  |  | 😾 | 
|  | Unveiling the Capabilities of Latent Diffusion Models for Classification of Lung Diseases in Chest X-Rays |  |  |  | 😾 | 
|  | Using Structural Similarity and Kolmogorov-Arnold Networks for Anatomical Embedding of Cortical Folding Patterns |  |  |  | 😾 | 
|  | Weakly Supervised Contrastive Learning for Better Severity Scoring of Lung Ultrasound |  |  |  | 😾 | 
|  | Weakly-Supervised Semantic Space Structuring: Cardiac Cycle Position for Cerebral Emboli Visualization Using Contrastive Learning |  |  |  | 😾 | 
|  | XLSTM-HVED: Cross-Modal Brain Tumor Segmentation and MRI Reconstruction Method Using Vision Xlstm and Heteromodal Variational Encoder-Decoder |  |  |  | 😾 | 
|  | Z-Stack Scanning can Improve AI Detection of Mitosis: A Case Study of Meningiomas |  |  |  | 😾 | 
|  | [18F]Florbetapir Pet Uptake Differences in White Matter Across the Alzheimer's Disease Continuum: From Cognitively Normal to MCI and Dementia |  |  |  | 😾 | 
</details>

<details>
  <summary><strong>ICML 2025</strong>（共筛选出 7 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 1W2WlYRq0K | [MindAligner: Explicit Brain Functional Alignment for Cross-Subject Visual Decoding from Limited fMRI Data](https://openreview.net/pdf?id=1W2WlYRq0K) | Yuqin Dai | Poster | 0 | [Code😻](https://github.com/Da1yuqin/MindAligner) | 
| EiAQrilPYP | [MindLLM: A Subject-Agnostic and Versatile Model for fMRI-to-text Decoding](https://openreview.net/pdf?id=EiAQrilPYP) | Weikang Qiu | Poster | 0 | [Code😻](https://github.com/Graph-and-Geometric-Learning/MindLLM) | 
| F3Cbhb611p | [Causal Tnvariance-aware Augmentation for Brain Graph Contrastive Learning](https://openreview.net/pdf?id=F3Cbhb611p) | Minqi Yu | Poster |  | [Code😻](https://github.com/BRAINML-GT/CausalBrainGraph) | 
| JiFfij5iv0 | [MedRAX: Medical Reasoning Agent for Chest X-ray](https://openreview.net/pdf?id=JiFfij5iv0) | Adibvafa Fallahpour | Poster | 5 | [Code😻](https://github.com/medrax-ai/MedRAX) | 
| ckbo9yutGO | [NeuroTree: Hierarchical Functional Brain Pathway Decoding for Mental Health Disorders](https://openreview.net/pdf?id=ckbo9yutGO) | Jun-En Ding | Poster |  | [Code😻](https://github.com/NeuroTree-MH/NeuroTree) | 
| pOAEfqa26i | [Learning Time-Varying Multi-Region Brain Communications via Scalable Markovian Gaussian Processes](https://openreview.net/pdf?id=pOAEfqa26i) | Weihan Li | Oral |  | [Code😻](https://github.com/brain-dynamics/MarkovianGP-Brain) | 
| yaI2ZYFmeD | [EEG-Language Pretraining for Highly Label-Efficient Clinical Phenotyping](https://openreview.net/pdf?id=yaI2ZYFmeD) | Sam Gijsen | Poster |  | [Code😻](https://github.com/EEG-Language-Pretraining/EEGLP) | 
</details>

<details>
  <summary><strong>ICLR 2025</strong>（共筛选出 14 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 0dELcFHig2 | [Multi-modal brain encoding models for multi-modal stimuli](https://openreview.net/pdf?id=0dELcFHig2) | SUBBA REDDY OOTA | Poster | 0 | [Code😻](https://github.com/subbareddy248/multi-modal-brain-stimuli) | 
| 10JOlFIPjt | [In vivo cell-type and brain region classification via multimodal contrastive learning](https://openreview.net/pdf?id=10JOlFIPjt) | Han Yu | Spotlight | 1 | [Code😻](https://ibl-nemo.github.io/) | 
| 28abpUEICJ | [CREIMBO: Cross-Regional Ensemble Interactions in Multi-view Brain Observations](https://openreview.net/pdf?id=28abpUEICJ) | Noga Mudrik | Spotlight | 0 | [Code😻](https://dandiarchive.org/dandiset/000363?search=mesoscale&pos=2) | 
| 3b9SKkRAKw | [LeFusion: Controllable Pathology Synthesis via Lesion-Focused Diffusion Models](https://openreview.net/pdf?id=3b9SKkRAKw) | Hantao Zhang | Spotlight | 0 | [Code😻](https://github.com/M3DV/LeFusion) | 
| 3xqqYOKILp | [BrainOOD: Out-of-distribution Generalizable Brain Network Analysis](https://openreview.net/pdf?id=3xqqYOKILp) | Jiaxing Xu | Poster | 0 | [Code😻](https://github.com/AngusMonroe/BrainOOD) | 
| 9UGfOJBuL8 | [Conditional Diffusion with Ordinal Regression: Longitudinal Data Generation for Neurodegenerative Disease Studies](https://openreview.net/pdf?id=9UGfOJBuL8) | Hyuna Cho | Spotlight | 0 | [Code😻](https://github.com/Hannah37/ConDOR-ICLR25) | 
| AD5yx2xq8R | [XAIguiFormer: explainable artificial intelligence guided transformer for brain disorder identification](https://openreview.net/pdf?id=AD5yx2xq8R) | Hanning Guo | Poster | 0 | [Code😻](https://github.com/HanningGuo/XAIguiFormer) | 
| CGON8Btleu | [BrainACTIV: Identifying visuo-semantic properties driving cortical selectivity using diffusion-based image manipulation](https://openreview.net/pdf?id=CGON8Btleu) | Diego García Cerdas | Poster | 0 | [Code😻](https://github.com/diegogcerdas/BrainACTIV) | 
| NPNUHgHF2w | [CBraMod: A Criss-Cross Brain Foundation Model for EEG Decoding](https://openreview.net/pdf?id=NPNUHgHF2w) | Jiquan Wang | Poster | 7 | [Code😻](https://github.com/wjq-learning/CBraMod) | 
| aWXnKanInf | [TopoLM: brain-like spatio-functional organization in a topographic language model](https://openreview.net/pdf?id=aWXnKanInf) | Neil Rathi | Oral | 2 | [Code😻](https://github.com/epflneuroailab/topolm) | 
| b57IG6N20B | [The Case for Cleaner Biosignals: High-fidelity Neural Compressor Enables Transfer from Cleaner iEEG to Noisier EEG](https://openreview.net/pdf?id=b57IG6N20B) | Francesco Carzaniga | Poster | 0 | [Code😻]( https://github.com/IBM/eeg-ieeg-brain-compressor) | 
| vgt2rSf6al | [MindSimulator: Exploring Brain Concept Localization via Synthetic fMRI](https://openreview.net/pdf?id=vgt2rSf6al) | Qi Zhang | Poster | 0 | 😾 | 
| yJ9QNbpMi2 | [Brain Mapping with Dense Features: Grounding Cortical Semantic Selectivity in Natural Images With Vision Transformers](https://openreview.net/pdf?id=yJ9QNbpMi2) | Andrew Luo | Poster | 1 | 😾 | 
| zZ6TT254Np | [Synthesizing Realistic fMRI: A Physiological Dynamics-Driven Hierarchical Diffusion Model for Efficient fMRI Acquisition](https://openreview.net/pdf?id=zZ6TT254Np) | Yufan Hu | Poster | 0 | [Code😻](https://github.com/CUHK-AIM-Group/PDH-Diffusion) | 
</details>

<details>
  <summary><strong>ICCV 2025</strong>（共筛选出 9 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 49 | [MorphoGen: Efficient Unconditional Generation of Long-Range Projection Neuronal Morphology via a Global-to-Local Framework](https://openaccess.thecvf.com/content/ICCV2025/papers/Zhu_MorphoGen_Efficient_Unconditional_Generation_of_Long-Range_Projection_Neuronal_Morphology_via_ICCV_2025_paper.pdf) | Tianfang Zhu | Poster |  | [Code😻](https://github.com/Brainsmatics/MorphoGen) | 
| 435 | [CABLD: Contrast-Agnostic Brain Landmark Detection with Consistency-Based Regularization](2411.17845) | Soorena Salari | Poster |  | [Code😻](https://github.com/HealthX-Lab/CABLD) | 
| 784 | [GEMeX: A Large-Scale, Groundable, and Explainable Medical VQA Benchmark for Chest X-ray Diagnosis](https://openaccess.thecvf.com/content/ICCV2025/papers/Liu_GEMeX_A_Large-Scale_Groundable_and_Explainable_Medical_VQA_Benchmark_for_ICCV_2025_paper.pdf) | Bo Liu | Poster |  | [Code😻](https://github.com/Awenbocc/GEMeX-Project) | 
| 940 | [Debiased Curriculum Adaptation for Safe Transfer Learning in Chest X-ray Classification](https://openaccess.thecvf.com/content/ICCV2025/papers/Liu_Debiased_Curriculum_Adaptation_for_Safe_Transfer_Learning_in_Chest_X-ray_ICCV_2025_paper.pdf) | Mingyang Liu | Poster |  | [Code😻](https://github.com/mingyangliu1024/DCA) | 
| 1188 | [Optimal Transport for Brain-Image Alignment: Unveiling Redundancy and Synergy in Neural Information Processing](2503.10663) | Yang Xiao | Poster |  | [Code😻](https://github.com/NKUShaw/OT-Alignment4brain-to-image) | 
| 1315 | [Neuroverse3D: Developing In-Context Learning Universal Model for Neuroimaging in 3D](2503.02410) | Jiesi Hu | Poster |  | [Code😻](https://github.com/jiesihu/Neuroverse3D) | 
| 1599 | [M-Net: MRI Brain Tumor Sequential Segmentation Network via Mesh-Cast](https://openaccess.thecvf.com/content/ICCV2025/papers/Lu_M-Net_MRI_Brain_Tumor_Sequential_Segmentation_Network_via_Mesh-Cast_ICCV_2025_paper.pdf) | Jiacheng Lu | Poster |  | 😾 | 
| 1961 | [Bridging the Gap between Brain and Machine in Interpreting Visual Semantics: Towards Self-adaptive Brain-to-Text Decoding](https://openaccess.thecvf.com/content/ICCV2025/papers/Chen_Bridging_the_Gap_between_Brain_and_Machine_in_Interpreting_Visual_ICCV_2025_paper.pdf) | Jiaxuan Chen | Poster |  | [Code😻](https://github.com/JxuanC/MindSA) | 
| 2179 | [Learn2Synth: Learning Optimal Data Synthesis Using Hypergradients for Brain Image Segmentation](2411.16719) | Xiaoling Hu | Poster |  | [Code😻](https://github.com/HuXiaoling/Learn2Synth) | 
</details>

<details>
  <summary><strong>CVPR 2025</strong>（共筛选出 8 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 32580 | [CheXwhatsApp: A Dataset for Exploring Challenges in the Diagnosis of Chest X-rays through Mobile Devices](https://openaccess.thecvf.com/content/CVPR2025/papers/Antony_CheXwhatsApp_A_Dataset_for_Exploring_Challenges_in_the_Diagnosis_of_CVPR_2025_paper.pdf) | Mariamma Antony | Poster | 0 | [Code😻](https://github.com/cheXwhatsApp/cheXwhatsApp) | 
| 33152 | [VasTSD: Learning 3D Vascular Tree-state Space Diffusion Model for Angiography Synthesis](2503.12758) | Zhifeng Wang | Poster | 0 | 😾 | 
| 33537 | [FactCheXcker: Mitigating Measurement Hallucinations in Chest X-ray Report Generation Models](https://openaccess.thecvf.com/content/CVPR2025/papers/Heiman_FactCheXcker_Mitigating_Measurement_Hallucinations_in_Chest_X-ray_Report_Generation_Models_CVPR_2025_paper.pdf) | Alice Heiman | Poster | 0 | [Code😻](https://github.com/rajpurkarlab/FactCheXcker) | 
| 33842 | [Incomplete Multi-modal Brain Tumor Segmentation via Learnable Sorting State Space Model](https://openaccess.thecvf.com/content/CVPR2025/papers/Zhang_Incomplete_Multi-modal_Brain_Tumor_Segmentation_via_Learnable_Sorting_State_Space_CVPR_2025_paper.pdf) | Zheyu Zhang | Poster | 0 | 😾 | 
| 34007 | [SuperLightNet: Lightweight Parameter Aggregation Network for Multimodal Brain Tumor Segmentation](https://openaccess.thecvf.com/content/CVPR2025/papers/Yu_SuperLightNet_Lightweight_Parameter_Aggregation_Network_for_Multimodal_Brain_Tumor_Segmentation_CVPR_2025_paper.pdf) | Feng Yu | Poster | 1 | [Code😻](https://github.com/WTU1020-Medical-Segmentation/SuperLightNet) | 
| 34086 | [Enhanced Contrastive Learning with Multi-view Longitudinal Data for Chest X-ray Report Generation](https://openaccess.thecvf.com/content/CVPR2025/papers/Liu_Enhanced_Contrastive_Learning_with_Multi-view_Longitudinal_Data_for_Chest_X-ray_CVPR_2025_paper.pdf) | Kang Liu | Poster | 3 | [Code😻](https://github.com/mk-runner/MLRG) | 
| 34267 | [Cross-Modal Interactive Perception Network with Mamba for Lung Tumor Segmentation in PET-CT Images](2503.17261) | Jie Mei | Poster | 0 | [Code😻](https://github.com/mj129/CIPA) | 
| 34348 | [KMD: Koopman Multi-modality Decomposition for Generalized Brain Tumor Segmentation under Incomplete Modalities](https://openaccess.thecvf.com/content/CVPR2025/papers/Liu_KMD_Koopman_Multi-modality_Decomposition_for_Generalized_Brain_Tumor_Segmentation_under_CVPR_2025_paper.pdf) | Tianyi Liu | Poster | 0 | [Code😻](https://github.com/T-Y-Liu/KMD) | 
</details>

<details>
  <summary><strong>AAAI 2025</strong>（共筛选出 14 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| article-32170 | [CA-MLIF: Cross-Attention and Multimodal Low-Rank Interaction Fusion Framework for Tumor Prognostic Prediction](https://ojs.aaai.org/index.php/AAAI/article/view/32170/34325) | Yajun An | Technical | 0 | [Code😻](https://github.com/panxipeng/PG-MLIF) | 
| article-32342 | [LLaVA Needs More Knowledge: Retrieval Augmented Natural Language Generation with Knowledge Graph for Explaining Thoracic Pathologies](https://ojs.aaai.org/index.php/AAAI/article/view/32342/34497) | Ameer Hamza | Technical | 1 | [Code😻](https://github.com/ailab-kyunghee/KG-LLaV A) | 
| article-32545 | [Semantic-guided Masked Mutual Learning for Multi-modal Brain Tumor Segmentation with Arbitrary Missing Modalities](https://ojs.aaai.org/index.php/AAAI/article/view/32545/34700) | Guoyan Liang | Technical | 0 | 😾 | 
| article-32611 | [See Through Their Minds: Learning Transferable Brain Decoding Models from Cross-Subject fMRI](https://ojs.aaai.org/index.php/AAAI/article/view/32611/34766) | Yulong Liu | Technical | 0 | [Code😻](https://github.com/YulongBonjour/STTM) | 
| article-32824 | [S³-Mamba: Small-Size-Sensitive Mamba for Lesion Segmentation](https://ojs.aaai.org/index.php/AAAI/article/view/32824/34979) | Gui Wang | Technical | 1 | [Code😻](https://github.com/ErinWang2023/S3-Mamba) | 
| article-32994 | [CLIP-MSM: A Multi-Semantic Mapping Brain Representation for Human High-Level Visual Cortex](https://ojs.aaai.org/index.php/AAAI/article/view/32994/35149) | Guoyuan Yang | Technical | 0 | [Code😻](https://github.com/BIT-YangLab/CLIP-MSM) | 
| article-33409 | [MEATRD: Multimodal Anomalous Tissue Region Detection Enhanced with Spatial Transcriptomics](https://ojs.aaai.org/index.php/AAAI/article/view/33409/35564) | Kaichen Xu | Technical | 0 | [Code😻](https://github.com/wqlzuel/MEATRD) | 
| article-33554 | [Wills Aligner: Multi-Subject Collaborative Brain Visual Decoding](https://ojs.aaai.org/index.php/AAAI/article/view/33554/35709) | Guangyin Bao | Technical | 0 | 😾 | 
| article-33572 | [Breaking Data Silos in Parkinson’s Disease Diagnosis: An Adaptive Federated Learning Approach for Privacy-Preserving Facial Expression Analysis](https://ojs.aaai.org/index.php/AAAI/article/view/33572/35727) | Meng Pang | Technical | 1 | 😾 | 
| article-33579 | [BrainGuard: Privacy-Preserving Multisubject Image Reconstructions from Brain Activities](https://ojs.aaai.org/index.php/AAAI/article/view/33579/35734) | Zhibo Tian | Technical | 1 | [Code😻](https://github.com/kunzhan/brainguard) | 
| article-33581 | [BrainMAP: Learning Multiple Activation Pathways in Brain Networks](https://ojs.aaai.org/index.php/AAAI/article/view/33581/35736) | Song Wang | Technical | 0 | [Code😻](https://github.com/LzyFischer/BrainMAP) | 
| article-34671 | [DECT: Harnessing LLM-assisted Fine-Grained Linguistic Knowledge and Label-Switched and Label-Preserved Data Generation for Diagnosis of Alzheimer’s Disease](https://ojs.aaai.org/index.php/AAAI/article/view/34671/36826) | Tingyu Mo | Technical | 0 | [Code😻]( https://github.com/SpeechCARE/LLMCare) | 
| article-34788 | [MEPNet: Medical Entity-Balanced Prompting Network for Brain CT Report Generation](https://ojs.aaai.org/index.php/AAAI/article/view/34788/36943) | Xiaodan Zhang | Technical | 0 | [Code😻](https://github.com/YanzhaoShi/MEPNet) | 
| article-35031 | [Accessible, At-Home Detection of Parkinson’s Disease via Multi-Task Video Analysis](https://ojs.aaai.org/index.php/AAAI/article/view/35031/37186) | Md Saiful Islam | Technical | 1 | [Code😻](https://github.com/ROC-HCI/UFNet) | 
</details>

<details>
  <summary><strong>IJCAI 2024</strong>（共筛选出 2 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| paper129 | [Self-Promoted Clustering-based Contrastive Learning for Brain Networks Pretraining](https://www.ijcai.org/proceedings/2024/0129.pdf) | Junbo Ma | Poster | 0 | [Code😻](https://github.com/brain-contrastive/SelfPromotedClustering) | 
| paper592 | [A Deep Probabilistic Spatiotemporal Framework for Dynamic Graph Representation Learning with Application to Brain Disorder Identification](https://www.ijcai.org/proceedings/2024/0592.pdf) | Sin-Yee Yap | Poster | 1 | [Code😻](https://github.com/brain-dynamics/DynamicGraphProbabilistic) | 
</details>

<details>
  <summary><strong>ECCV 2024</strong>（共筛选出 8 篇论文）</summary>

  <br/>

| ID | Title | Authors | Status | Citations | AnyCode |
| -- | ----- | ------- | :----: | :-------: | :-----: |
| 326 | [NeuroPictor: Refining fMRI-to-Image Reconstruction via Multi-individual Pretraining and Multi-level Modulation](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/06746.pdf) | Jingyang Huo | Poster | 14 | [Code😻](https://github.com/jingyanghuo/neuropictor) | 
| 720 | [MinD-3D: Reconstruct High-quality 3D objects in Human Brain](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/06365.pdf) | Jianxiong Gao | Poster | 9 | [Code😻](https://github.com/JianxGao/MinD-3D) | 
| 1109 | [UMBRAE: Unified Multimodal Brain Decoding](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/01133.pdf) | Weihao Xia | Poster | 12 | [Code😻](https://github.com/weihaox/UMBRAE) | 
| 1166 | [Brain-ID: Learning Contrast-agnostic Anatomical Representations for Brain Imaging](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/01918.pdf) | Peirong Liu | Poster | 5 | [Code😻](https://github.com/peirong26/Brain-ID) | 
| 1456 | [Brain-ID: Learning Contrast-agnostic Anatomical Representations for Brain Imaging](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/03816.pdf) | Camilo Fosco | Poster | 2 | [Code😻](https://github.com/peirong26/Brain-ID) | 
| 1461 | [VSViG: Real-time Video-based Seizure Detection via Skeleton-based Spatiotemporal ViG](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/10768.pdf) | Yankun Xu | Poster | 2 | [Code😻](https://github.com/xuyankun/VSViG/) | 
| 1543 | [ChEX: Interactive Localization and Region Description in Chest X-rays](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/03114.pdf) | Philip Müller | Poster | 6 | [Code😻](https://github.com/philip-mueller/chex) | 
| 2285 | [GenerateCT: Text-Conditional Generation of 3D Chest CT Volumes](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/10147.pdf) | Ibrahim Ethem Hamamci | Poster | 24 | [Code😻](https://github.com/ibrahimethemhamamci/GenerateCT) | 
</details>
<!-- LB_PAPERS_END -->

## ✅ Med-Multimodal-Paper-List
聚焦于**多模态/跨模态医学研究**的论文
<!-- PAPERS_START -->


<!-- PAPERS_END -->
